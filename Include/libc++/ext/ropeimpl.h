...
class _Rope_flatten_char_consumer : public _Rope_char_consumer<_CharT> {
	...
	bool operator() (const _CharT* __leaf, size_t __n) {
	    uninitialized_copy_n(__leaf, __n, this->_M_buf_ptr);
	    _M_buf_ptr += __n;
	    return true;
	}
};
...
rope<_CharT,_Alloc>::_S_balance(_RopeRep* __r)
{
    ...
    // __forest[__i].this->_M_size >= _S_min_len[__i]
...
_Rope_char_ref_proxy<_CharT, _Alloc>::operator= (_CharT __c) {
    _RopeRep* __old = this->_M_root->_M_tree_ptr;
#   ifndef __GC
...
#   endif
    this->_M_root->_M_tree_ptr = __result;
    return *this;
}
...
inline _Rope_char_ref_proxy<_CharT, _Alloc>::operator _CharT () const
{
	...
	return _M_current;
        ...
        return _My_rope::_S_fetch(this->_M_root->_M_tree_ptr, _M_pos);
    }
}
template <class _CharT, class _Alloc>
...
const _CharT* rope<_CharT,_Alloc>::replace_with_c_str() {
    ...
    _M_tree_ptr = _S_new_RopeLeaf(__result, __s, this->get_allocator());
    return(__result);
}
