...
#define REG16(addr)	(*(volatile WORD*)(addr))
#define REG32(addr)	(*(volatile DWORD*)(addr))

#endif /* !ASSEMBLY */

#include "jz4750laic.h"
#include "jz4750ldma.h"
#include "jz4750lcodec.h"
#include "jz4750lcmp.h"
...
#define IRQ_SSI 		26
#define IRQ_UDC			27
#define IRQ_DMA1		28		// For DMA channel 4-7
#define IRQ_DMA0		29		// For DMA channel 0-3
#define IRQ_IPU			30
#define IRQ_LCD  		31
...
#define CPM_MSCCDR	(CPM_BASE+0x68) /* MSC0 & MSC1 share device clock divider Register */
...
#define CPM_CLKGR_AIC1		CPM_CLKGR_AIC
#define CPM_CLKGR_AIC2		CPM_CLKGR_AIC
...
#define DMAC_DCKER(m)			(DMAC_BASE + 0x0310 + 0x100 * (m))              // DMA Clock Enable  Register 
...
#define REG_DMAC_DCKER(m)		REG32(DMAC_DCKER(m))
...
#define AIC_CKCFG		(AIC_BASE + 0x0A0)
#define	AIC_RGADW		(AIC_BASE + 0x0A4)
#define AIC_RGDAT		(AIC_BASE + 0x0A8)
...
#define REG_AIC_CKCFG		REG32(AIC_CKCFG)
#define REG_AIC_REGDW		REG32(AIC_RGADW)
#define REG_AIC_REGAT		REG32(AIC_RGDAT)
...
#define AIC_FR_LSMP		(1 << 6)	/*Play ZERO sample when TX FIFO underflow OR Play last sample when TX FIFO underflow */
...
#define SSI_CR1_FMAT_MASK	(0x3 << SSI_CR1_FMAT_BIT)
  #define SSI_CR1_FMAT_SPI	  (0 << SSI_CR1_FMAT_BIT) /* Motorola隆炉s SPI format */
 ...
 * MSC
 *************************************************************************/
#define 	MSC_BASE(x)		(MSC0_BASE + (x) * 0x1000)

#define	MSC_CTRL(x)			(MSC_BASE(x) + 0x000)
...
#define	REG_MSC_CTRL(x)		REG16(MSC_CTRL(x))
...
#define	REG_MSC_LPM(x)			REG8(MSC_LPM(x))
...
#define SADC_ADFLT	(SADC_BASE + 0x24)  /* ADC PBAT Data Register */
#define SADC_ADCLK	(SADC_BASE + 0x28)  /* ADC SADCIN Data Register */
...
#define REG_SADC_ADFLT		REG32(SADC_ADFLT)
...
/*
 * MSC0_CLK----->PC8(PFUN = 1	PTRG = 0 	PSEL = 1)
 * MSC0_CMD---->PC9 (PFUN = 1	PTRG = 0		PSEL = 1)
 * MSC0_D3------>PC22(PFUN = 1	PTRG = 0		PSEL = 1)
 * MSC0_D0------>PC28(PFUN = 1	PTRG = 0		PSEL = 1)
 * MSC0_D1------>PC29(PFUN = 1	PTRG = 0		PSEL = 1)
 * MSC0_D2------>PC27(PFUN = 1	PTRG = 0		PSEL = 0)
 */
#define __gpio_as_msc0_4bit()			\
do {						\
	/*MSC0_CLK MSC0_CMD MSC0_D3 MSC0_D0 MSC0_D1*/\
	REG_GPIO_PXFUNS(2) = 0x30400300;	\
	REG_GPIO_PXTRGC(2) = 0x30400300;	\
	REG_GPIO_PXSELS(2) = 0x30400300;	\
	REG_GPIO_PXPES(2)   = 0x30400300;	\
	/*MSC0_D2*/						\
	REG_GPIO_PXFUNS(2) = 0x08000000;	\
	REG_GPIO_PXTRGC(2) = 0x08000000;	\
	REG_GPIO_PXSELC(2)  = 0x08000000;	\
	REG_GPIO_PXPES(2)   = 0x08000000;	\
} while (0)
...
#define __gpio_as_msc 	__gpio_as_msc0_4bit /* default as msc0 8bit */
#define __gpio_as_msc0 	__gpio_as_msc0_4bit /* msc0 default as 8bit */
...
#define __dmac_fast_msc_enable_module(m) \
	( REG_DMAC_DMACR(m) |= DMAC_DMACR_FMSC )
#define __dmac_fast_msc_disable_module(m) \
	( REG_DMAC_DMACR(m) &= ~DMAC_DMACR_FMSC )
...
#define __aic_set_esclk()		( REG_AIC_FR |= AIC_FR_LSMP )
#define __aic_clr_esclk()		( REG_AIC_FR &= ~AIC_FR_LSMP )
