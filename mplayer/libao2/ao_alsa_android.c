...
static int try_open_device(const char *device, int open_mode, int try_ac3)
{
  ...
  if (!try_ac3 || err < 0) {
	  printf("%s:%s:%d  device=%s  stream=%d  open_mode=%d\n",__FILE__,__FUNCTION__,__LINE__,device,SND_PCM_STREAM_PLAYBACK,open_mode);
    err = snd_pcm_open(&alsa_handler, device, SND_PCM_STREAM_PLAYBACK,
		       open_mode);
  }
  printf("\n\n\nao_alsa.c open err=%d\n\n\n",err);
  return err;
}
...
static int init(int rate_hz, int channels, int format, int flags)
{
    ...
    /* only for Android mixing */
    if (!strcmp(device.str, "default")) {
	    printf("\nSelect the best dmix plugin\n");
	    switch (rate_hz) {
	    case 8000:
		    device.str = "plug:dmix_8k";
		    break;
	    case 9600:
		    device.str = "plug:dmix_9k";
		    break;	    
	    case 11025:
		    device.str = "plug:dmix_11k";
		    break;	    
	    case 12000:
		    device.str = "plug:dmix_12k";
		    break;	    
	    case 16000:
		    device.str = "plug:dmix_16k";
		    break;
	    case 22050:
		    device.str = "plug:dmix_22k";
		    break;
	    case 24000:
		    device.str = "plug:dmix_24k";
		    break;
	    case 32000:
		    device.str = "plug:dmix_32k";
		    break;
	    case 44100:
		    //device.str = "plug:dmix_44k";
		    device.str = "plug:AndroidPlayback";
		    break;
	    case 48000:
		    device.str = "plug:dmix_48k";
		    break;
	    case 96000:
		    device.str = "plug:dmix_96k";
		    break;	    
	    default:
		    printf("no %d Hz dmix plugin, so use 44100KHz dmix plugin!\n");
		    device.str = "plug:dmix_44k";
	    }
    } else {
	    printf("Bad, only use default device for alsa!\n");
    }
