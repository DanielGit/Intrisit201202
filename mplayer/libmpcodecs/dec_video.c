...
extern int64_t video_time_usage;
extern int64_t vout_time_usage;
...
#ifndef __LINUX__
{
//	    kprintf ("mpcodecs_vd_drivers[i]->info->short_name = %s, sh_video->codec->drv = %s \n",
//                   mpcodecs_vd_drivers[i]->info->short_name,sh_video->codec->drv);
...
#undef printf
        ...
        {
          codecs_t *p;
          p = copy_codecs_script (sh_video->codec);
          if (!p)
          {
            mpvdec=NULL;
            break;
	...
	if (mpvdec)
	{
          if (!strcmp("ffmpeg", sh_video->codec->drv))
            load_plugin (sh_video->codec->drv, sh_video->codec->dll, 1, 0);
        }
	else
        {
...
//          kprintf ("+++ name = %s +++\n", sh_video->codec->drv);
#ifndef __LINUX__
#if defined (JZ4750_OPT) || defined (JZ4755_OPT)
          if (!strcmp("realvid", sh_video->codec->drv))
          {
             unsigned char* extrahdr=(unsigned char*)(sh_video->bih+1);
             unsigned int extrahdr_size = sh_video->bih->biSize - sizeof(BITMAPINFOHEADER);
             unsigned int real_id = be2me_32(((unsigned int*)extrahdr)[1]);
             unsigned int id_major = (real_id >> 28) & 0xF;
             unsigned int id_minor = (real_id >> 20) & 0xFF;
             unsigned int isRv8 = (id_major == 3 && (id_minor == 2 || id_minor == 128));
             if (isRv8)
               funcs_sym = load_plugin ("realvid_rv8", "", 1, 0);
             else
               funcs_sym = load_plugin (sh_video->codec->drv, "", 1, 0);
          }
          else
#endif
#endif
...
//unsigned int Decode_Time_10US = 0;
...
//	kprintf("decode_video start: in_size = %d\n",in_size);
...
//	kprintf("video decode end: in_size = %d\n",in_size);
    mpi = mpvdec->decode(sh_video, start, in_size, drop_frame);
//	kprintf("video decode end: mpi = %x\n",	mpi);
    //------------------------ frame decoded. --------------------
    ...
    t2 = GetTimer(); t = t2-t;
    //Decode_Count++;
    //Decode_Time_10US +=  (t / 10);  //for demuxer cal rate
    video_time_usage += t;
