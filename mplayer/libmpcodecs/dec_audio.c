...
int init_audio_codec(sh_audio_t *sh_audio)
{
	if ((af_cfg.force & AF_INIT_FORMAT_MASK) == AF_INIT_FLOAT) 
	{
		...
		if (sh_audio->ad_driver->control(sh_audio, ADCTRL_QUERY_FORMAT,&fmt) == CONTROL_TRUE) 
		{
	...
	if(sh_audio->audio_in_minsize>0)
	{
		...
		sh_audio->a_in_buffer=memalign(32,sh_audio->a_in_buffer_size);
	...
	sh_audio->a_buffer=memalign(32,sh_audio->a_buffer_size);
	...
	sh_audio->i_bps*8*0.001,((float)sh_audio->i_bps/sh_audio->o_bps)*100.0,sh_audio->i_bps,sh_audio->o_bps);
...
int init_audio(sh_audio_t *sh_audio,char* codecname,char* afm,int status)
{
	...
	while(1)
	{
		...
		if(!(sh_audio->codec=find_audio_codec(sh_audio->format,sh_audio->wf?(&i):NULL, sh_audio->codec, force) ))
			break;
		F("sh_audio->codec->drv = %s",sh_audio->codec->drv);
		...
		if(codecname && strcmp(sh_audio->codec->name,codecname))
			continue; // -ac
#ifdef JZ47_OPT
//         if (sh_audio->samplerate > 0 && sh_audio->samplerate < 32000
//             && !strcmp(sh_audio->codec->drv,"libmad")) continue; // madlib > 32K
#endif
		F("2");
		...
		{
#ifdef JZ4740_MTV 	
			if (mpcodecs_ad_drivers[i] == (void *)1)
				continue;
#endif			
			F ("mpcodecs_ad_drivers[i]->info->short_name = %s, sh_audio->codec->drv = %s",
				mpcodecs_ad_drivers[i]->info->short_name,sh_audio->codec->drv);
...
#ifndef __LINUX__
		ptr_flag = sh_audio->codec;
		{
			codecs_t *p;
			p = copy_codecs_script (sh_audio->codec);
			if (!p)
			{
		F("7");
				mpadec=NULL;
		...
		{
		F("8");
			if (!strcmp("ffmpeg", sh_audio->codec->drv))
			{
		F("9");
				load_plugin (sh_audio->codec->drv, sh_audio->codec->dll, 0, 0);
			}
		}
		else
		{
		...
		F("10");
			funcs_sym = load_plugin (sh_audio->codec->drv, "", 0, 0);
