...
vd_functions_t *vd_libmpeg2_table (void)
{
   mpeg2_init_var();
  return &mpcodecs_vd_libmpeg2;
}
#endif
//#include "libvo/video_out.h"	// FIXME!!!

#if defined(JZ4725_OPT)
#include "libmpeg2_jz4740/mpeg2.h"
#include "libmpeg2_jz4740/attributes.h"
#include "libmpeg2_jz4740/mpeg2_internal.h"
#else
...
static mp_image_t* decode(sh_video_t *sh,void* data,int len,int flags){
    ...
    mpeg2dec_t * mpeg2dec = context->mpeg2dec;
    const mpeg2_info_t * info = mpeg2_info (mpeg2dec);
    int drop_frame = 0, framedrop=flags&3;
	...
	case STATE_PICTURE:
...
#if DROP_IMAGE
	    static int count = 0;
	    static int perframe_count = 0;
	    static int dropcount = 0;
	    static int dropframecount = 0;
	     //framedrop = 0;
	    if(perframe_count == 0)
	    {	
	    	//Í³¼ÆI Ö¡
	    ...
	    }else
	    {
		    switch(mpeg2dec->decoder.coding_type)
		    {
		    	...
		    	//	break;
		    	case I_TYPE:
		    		dropcount -= dropframecount;
				    dropframecount = 0;
				    drop_to_IFrame = 0;
				  	//if(dropcount >= perframe_count)
			    	//	framedrop = 2;
			   	  //else
			   	  //	framedrop = 0;
			   	  if(count > perframe_count) 
			    		perframe_count = count; 
			    	count = 0;
		    		break;
		    	default:
		    		if(framedrop) dropcount++;
		    		if(dropcount >= perframe_count - count)
		    		{
		    			framedrop = 2;
			    		dropframecount++;
		    		}		
	    ...
	    count++;
#endif
	   //if(framedrop == 2) return NULL;
	   //F("drop_drame = %d,perframe_count = %d,dropcount = %d\n",drop_frame,perframe_count,dropcount);
	     
	    
	    //drop_frame = framedrop && (mpeg2dec->decoder.coding_type == B_TYPE);
	    drop_frame = 0;
	    //F("mpeg2dec->decoder.coding_type = %x framedrop = %d dropcount = %d\n",
	    //mpeg2dec->decoder.coding_type,framedrop,dropcount);
