...
#define ERROR() { \
  ...
  for(i=0; i<3; i++) {
    ...
    while(*ptr){
#ifdef USE_16M_SDRAM
      printf ("+++++ read_vorbis_comment: ptr = %s +++++\n", *ptr);
#endif
...
static int decode_audio(sh_audio_t *sh,unsigned char *buf,int minlen,int maxlen)
{
	...
	while(len < minlen) {
	  ...
	  while(samples>0){
	    ...
	    for(i=0;i<CHAN_NUM (ov->vi.channels);i++){
		...
		*ptr=val;
	    ...
	    len+=2*CHAN_NUM (ov->vi.channels)*bout;
...
void install_mpcodecs_vorbis ()
{
  int i;
  ad_functions_t *p;

  p = mpcodecs_ad_drivers [0];
  for (i = 1; p != NULL; i++)
  {
    if (p == ((void *)1))
    {
      mpcodecs_ad_drivers[i - 1] = &mpcodecs_ad_libvorbis;
      break;
    }
    else
      p = mpcodecs_ad_drivers [i];
  }
