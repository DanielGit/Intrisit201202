...
#ifdef JZ4740_IPU
// fix the outfmt for xvid (use IYUV to replace BGR16)
    if (!strcmp(sh->codec->drv, "xvid"))
    {
      for(i=0;i<CODECS_MAX_OUTFMT;i++)
        sh->codec->outfmt[i] = 0xffffffff;
      sh->codec->outfmt[0] = 0x30323449;
    }

    j=-1;
// Don't use the vf_vo for jz47 ipu convert
    if(strcmp(vf->info->name,"vo"))
#endif
    for(i=0;i<CODECS_MAX_OUTFMT;i++){
	int flags;
