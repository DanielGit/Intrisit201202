...
//-------------------------------------------------------------
#define INDEX_MAX_SIZE ((8 * 1024 * 1024) >> 4) 
static inline void ConvertIndex(avi_priv_t* priv,int size)
{
	int i;
	for (i = 0; i < size; i++) {	// swap index to machine endian
...
//#undef printf
static inline void ReadIndex(demuxer_t *demuxer,int pos)
{
	avi_priv_t* priv=demuxer->priv;
	//F("+++ %d %x %d\n",pos,priv->idx,priv->idx_size);
	
	if(priv->idx == 0)
	{	
		if((priv->idx_size) < INDEX_MAX_SIZE)    
    {
    	 priv->idx=malloc(priv->idx_size<<4); 	
    	 stream_read(demuxer->stream,(char*)priv->idx,priv->idx_size<<4);
       ConvertIndex(priv,priv->idx_size);
       priv->idx_idx = 0;
       return;
    }else
    {
    	priv->idx=malloc((INDEX_MAX_SIZE << 4));
    	stream_read(demuxer->stream,(char*)priv->idx,(INDEX_MAX_SIZE << 4));
   		ConvertIndex(priv,INDEX_MAX_SIZE);
   		priv->idx_idx = 0;
    }
  }else
  {
  	int curpos = stream_tell(demuxer->stream);
  	//F("seek = %d %d %d\n",priv->idx_offset,priv->m_idx_offset,priv->m_idx_offset + (pos << 4));
  	stream_seek(demuxer->stream,priv->m_idx_offset + (pos << 4));
  	if((pos + INDEX_MAX_SIZE) < priv->idx_size)
  		stream_read(demuxer->stream,(char*)priv->idx,(INDEX_MAX_SIZE << 4));
  	else
  		stream_read(demuxer->stream,(char*)priv->idx,(priv->idx_size - pos) << 4);
   	//int i = 0;
   	//unsigned char *cc = (unsigned char *)priv->idx;
   	//F("aaaaaaaaaaaaaaaaaaaaaaa\n");
   	//for(i;i < 16;i++)
   	//	printf("0x%02x ",cc[i]);
   	
   	priv->idx_idx = pos / INDEX_MAX_SIZE;
   	ConvertIndex(priv,INDEX_MAX_SIZE);
  	//F("seek = %d\n",curpos);
  	stream_seek(demuxer->stream,curpos);
  }
  //F("--- %d\n",pos);
}
...
//			F("\n");
	...
	//printf("pos = %d pos / INDEX_MAX_SIZE = %d priv->idx_idx = %d\n",pos,pos / INDEX_MAX_SIZE,priv->idx_idx);
	if((pos / INDEX_MAX_SIZE) != priv->idx_idx)
	{
		
		ReadIndex(demuxer,pos / INDEX_MAX_SIZE * INDEX_MAX_SIZE);
	}
	return &((AVIINDEXENTRY *)priv->idx)[pos % INDEX_MAX_SIZE];
  #else
  return &((AVIINDEXENTRY *)priv->idx)[pos];
  #endif
...
//        if(demuxer->audio->id==-1) demuxer->audio->id=stream_id;
    ...
    if(index_mode && !priv->isodml){
      ...
      /* Add Index */
      priv->m_idx_offset = stream_tell(demuxer->stream);
      priv->idx = 0;
      ReadIndex(demuxer,0);
