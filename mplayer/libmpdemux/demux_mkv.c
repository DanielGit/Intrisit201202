                  ...
                  uint32_t size, int block_bref)
{
  ...
  *hdr++ = actual_size;                  // length of actual data
  *hdr++ = REALHEADER_SIZE + isize;    // offset to chunk offset array
  *hdr++ = block_bref ? 0 : 0x10;
...
handle_video_bframes (demuxer_t *demuxer, mkv_track_t *track, uint8_t *buffer,
                      uint32_t size, int block_bref, int block_fref, int keyframe)
{
  mkv_demuxer_t *mkv_d = (mkv_demuxer_t *) demuxer->priv;
  demux_packet_t *dp;
      ...
      for (i=0; i < laces; i++)
        {
          if (ds == demuxer->video && track->realmedia)
            ...
            handle_video_bframes (demuxer, track, block, lace_size[i],
                                  block_bref, block_fref, (flags&0x80));
          else
            {
...
{  demux_stream_t *vs =demux->video;
  ...
  while(1){
    if(dp == NULL) return count+300; //keyframe not be found in this link of pack 
    count++;
    if(dp->flags & 0x10){
...
demux_mkv_control (demuxer_t *demuxer, int cmd, void *arg)
{
  ...
  switch (cmd)
	...
	case DEMUXER_CTRL_GET_KEYFRAME_TIME:
	{
	   ...
	   *(float *)arg = ((float)(count) * mkv_d->tracks[0]->default_duration);				
		return DEMUXER_CTRL_OK;
	}
