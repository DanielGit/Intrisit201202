...
static mp3_hdr_t *add_mp3_hdr(mp3_hdr_t **list, off_t st_pos,
	...
	{
		if (((*list)->next_frame_pos < st_pos) || 
			((*list)->mp3_chans != mp3_chans)  || 
			((*list)->mp3_freq != mp3_freq)    || 
			((*list)->mpa_layer != mpa_layer) ) 
		{
			// wasn't valid!
			tmp = (*list)->next;
			free(*list);
...
static uint32_t mp3_total_framecount(demuxer_t* demuxer)
{
	...
	while(s->eof == 0) 
	{
		...
		len = mp_get_mp3_header(hdr, &mp3_chans, &mp3_freq,&mpa_spf, &mpa_layer, &mpa_br);
		if(len < 0) 
		{
			pos_start++;
		...
		{	//如果MP3大于2M，则只检查1/8的from数据,然后进行估算
			if( (stream_tell(s) - pos_start) > ((file_len - pos_start)>>3))
			{
				count = count << 3;
				break;
			}
...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  while(n < 250000 && !s->eof) 
  {
	...
	else if((mp3_flen = mp_get_mp3_header(hdr, &mp3_chans, &mp3_freq,&mpa_spf, &mpa_layer, &mpa_br)) > 0) 
	{
		mp3_found = add_mp3_hdr(&mp3_hdrs, st_pos, mp3_chans, mp3_freq, mpa_spf, mpa_layer, mpa_br, mp3_flen);
		if (mp3_found) 
		{
			frmt = MP3;
			break;
  ...
  F("frm = %d,st_pos = %d, n= %d",frmt,st_pos,n);
  ...
  case MP3:
	...
	F("frame_pos = %d",mp3_found->frame_pos);
...
#ifndef __LINUX__
      //F("get totla time %d %d",sh_audio->audio.dwRate,sh_audio->wf->nBlockAlign);
    ...
    {	
    		...
    		mp3_totaltime = (float)playertime / 1000.0;  
     //F("mp3_totaltime = %d count = %d",(unsigned int)(mp3_totaltime *1000.0),dcount);
     //F("s->eof = %d",s->eof);
    free(mp3_found);
    mp3_found = NULL;
    //F("BBBBB");
...
//	F("demuxer->movi_end = %d demuxer->movi_start = %d sh_audio->i_bps = %d",demuxer->movi_end,demuxer->movi_start,sh_audio->i_bps);
...
static int demux_audio_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds) {
  ...
  case MP3 :
    while(1) 
	{
		...
		l = mp_get_mp3_header(hdr, &mp3_chans, &mp3_freq,&mpa_spf, &mpa_layer, &mpa_br);
		...
		else 
		{
			...
			priv->next_pts += (double)mpa_spf / (double)mp3_freq;
			break;
		}
    } 
	break;
...
static void demux_audio_seek(demuxer_t *demuxer,float rel_seek_secs,float audio_delay,int flags){
	...
	F("demux_audio_seek: priv->frmt = %d , flag = %d",priv->frmt, flags);
	if(priv->frmt == MP3 && hr_mp3_seek && !(flags & 2)) 
	{
		F("rel_seek_secs = %d, priv->next_pts = %d,rel_seek_secs = %d",(int)(rel_seek_secs*1000) , (int)(priv->next_pts*1000),(int)(rel_seek_secs*1000));
		len = (flags & 1) ? rel_seek_secs - priv->next_pts : rel_seek_secs;
		F("len = %d",(int)(len*1000));
		if(len < 0) 
		{
		  stream_seek(s,demuxer->movi_start);
	...
	F("pos = %d, movi_start = %d, movi_end = %d",pos,demuxer->movi_start,demuxer->movi_end);
...
static int demux_audio_control(demuxer_t *demuxer,int cmd, void *arg){
    ...
    float audio_length;
    if(mp3_totaltime > 0)
    	audio_length = mp3_totaltime;
    else	
    	audio_length = (float)(demuxer->movi_end - demuxer->movi_start)/ sh_audio->i_bps;
