...
#ifndef __LINUX__
extern demuxer_t* (*init_avi_with_ogg_funcp)(demuxer_t* demuxer);
extern int (*demux_ogg_open_funcp)(demuxer_t* demuxer);
extern demuxer_desc_t* demuxer_list[];
void install_demuxer_ogg ()
{
  int i;
  demuxer_desc_t* p;
  p = demuxer_list[0];
  for (i = 1; p != NULL; i++) 
  {
    if (p == ((void *)1))
    {
      demuxer_list[i - 1] = &demuxer_desc_ogg;
      break;
    }
    else
      p = demuxer_list[i];
  ...
  init_avi_with_ogg_funcp  = init_avi_with_ogg;
  demux_ogg_open_funcp = demux_ogg_open;
}
#endif
