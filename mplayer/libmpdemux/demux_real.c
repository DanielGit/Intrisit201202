...
#ifdef JZ47_OPT
static int verify_index_chunk (demuxer_t *demuxer)
{
    real_priv_t *priv = demuxer->priv;
    int origpos = stream_tell(demuxer->stream);
    int next_header_pos = priv->index_chunk_offset;
    int i, entries, stream_id, prev_timestamp, timestamp, offset;
...
verify_read_index:
    ...
    for (i = 0; i < entries; i++)
    {
	    ...
	    offset = stream_read_dword(demuxer->stream);
		  ...
		  if (prev_timestamp && ((timestamp < prev_timestamp) || (timestamp - prev_timestamp) < 5))
		  {
			  stream_seek(demuxer->stream, origpos);
			  return 0;
		  }
...
static int parse_index_chunk(demuxer_t *demuxer)
{
    ...
    {
    	kprintf ("+++ verify index chunk failed ! \n");
    	return 0;
    }
#endif
read_index:
    ...
    for (i = 0; i < entries; i++)
    {
		...
		//F("offset = %d %d",i,priv->index_table[stream_id][i].offset);
		stream_skip(demuxer->stream, 4); /* packetno */
//priv->index_table[stream_id][i].packetno = stream_read_dword(demuxer->stream);
		//unsigned int packetno = stream_read_dword(demuxer->stream);
	
		//printf("Index table: Stream#%d: entry: %d: pos: %d :timestamp: %d packetno: %d\n",
		//    stream_id, i, priv->index_table[stream_id][i].offset,priv->index_table[stream_id][i].timestamp,packetno);
    }
...
static int generate_index(demuxer_t *demuxer)
{
    ...
    unsigned int curtimestamp = -1;
    int vstream_id = demuxer->video->id;
...
//	    priv->index_table_size[stream_id] * sizeof(real_index_table_t));
	...
	if(savetimestamp != -1)
		priv->duration = (curtimestamp - savetimestamp) / 1000;
    if (i == -256)
		stream_reset(demuxer->stream);
    stream_seek(demuxer->stream, origpos);
    if (i == -256)
...
//    if(v_pts<priv->v_pts || !kf) v_pts=priv->v_pts+frametime;
    priv->v_pts=v_pts;
    //mp_msg(MSGT_DEMUX,MSGL_INFO,"%4.2f fps\n",v_pts);
    return v_pts;
}
...
//---------------------------------------------------------------------------
#define FIND_KEYFRAME_ERROR -1
static int cur_timestamp = 0;

static int skip_keyframe_count = 0;
static int cur_skip_keyframe = 0;

static real_index_table_t* FindNextKeyFrameIdx(demuxer_t * demuxer)
{
	real_priv_t *priv = demuxer->priv;
	int vstream_id = demuxer->video->id;
	int cur_idx_pos = priv->current_vpacket;
	int m_cur_skip_keyframe = cur_skip_keyframe;
	 
	 
	real_index_table_t* idx;
	 
	idx = &priv->index_table[vstream_id][cur_idx_pos];
	while(cur_idx_pos < priv->index_table_size[vstream_id] - 1)
	{
	 	cur_idx_pos++;
	 	idx = &priv->index_table[vstream_id][cur_idx_pos];
...
static inline int FindSkipKeyFrameTime(demuxer_t * demuxer,int min_timestamp)
{
	real_priv_t *priv = demuxer->priv;
	int vstream_id = demuxer->video->id;
	int cur_idx_pos = priv->current_vpacket;
	...
	idx = &priv->index_table[vstream_id][cur_idx_pos];
	//printf("cur_idx_pos = %d\n",cur_idx_pos);
	//printf("priv->index_table_size[vstream_id] = %d\n",priv->index_table_size[vstream_id]);
	unsigned int cur_timestamp = idx->timestamp;
	while(cur_idx_pos < priv->index_table_size[vstream_id] - 1)
	{
	 	//F("cur_idx_pos = %d \n",cur_idx_pos);
		...
		//printf("idx->timestamp - cur_timestamp = %d min_timestamp = %d\n",idx->timestamp - cur_timestamp,min_timestamp);
	 	if(min_timestamp < 0) break;       
	 	unsigned int timestamp = idx->timestamp - cur_timestamp;
	 	if((timestamp > (unsigned int)min_timestamp) && (timestamp < 4000))
	 	{
	 		break;
	 	}
	 	skip_keyframe_count++;
...
static int demux_real_fill_buffer(demuxer_t *demuxer, demux_stream_t *dsds)
{
	...
	while(!stream_eof(demuxer->stream)){
		...
		if ((version==0x4441) && (len==0x5441)) { // new data chunk
			F("demux_real: New data chunk is coming!!!\n");
		...
		if (len < 12){
			...
			{	
				...
				int d_vp = priv->current_vpacket;
				//F("aaaa %d\n",priv->current_vpacket);
				if(d_vp >= priv->index_table_size[demuxer->video->id] - 1)
					return 0;
				//下一个	
				d_vp++;
				priv->current_vpacket++;
				int pos = priv->index_table[demuxer->video->id][d_vp].offset;
		...
		got_audio:
			...
			// parse audio chunk:
			{
				...
				if ((priv->intl_id[stream_id] == mmioFOURCC('I', 'n', 't', '4')) ||
					...
					//(user_correct_pts > 0 ? MP_NOPTS_VALUE : 0)
					priv->audio_timestamp[priv->sub_packet_cnt] = (priv->a_pts==timestamp) ? 0 : (timestamp/1000.0);
					priv->a_pts = timestamp;
			...
			{	
				...
				{	 
					...
					if(len > 2)
					{
						...
						m_vpkg_header=stream_read_char(demuxer->stream); --len;
					}	
					stream_skip(demuxer->stream, len);
					if(m_vpkg_header & 0x80)
					{	
						//printf("skip flags = %d\n",flags);
						goto fill_finish;
...
//		printf("xxx len=%d  \n",len);
					...
					if(ds->asf_packet){
						...
						//extra=(unsigned char*)(dp->buffer+dp_hdr->chunktab);
						...
						if(ds->asf_seq!=vpkg_seqnum){
							...
							//extra[2*dp_hdr->chunks + 0]=1;
							//extra[2*dp_hdr->chunks + 1]=0;
							//extra[2*dp_hdr->chunks + 2]=0;
							//extra[2*dp_hdr->chunks + 3]=0;
			
							//extra[2*dp_hdr->chunks + 4]=dp_hdr->len & 0x0ff;
							//extra[2*dp_hdr->chunks + 5]=(dp_hdr->len >> 8) & 0x0ff;
							//extra[2*dp_hdr->chunks + 6]=(dp_hdr->len >> 16) & 0x0ff;
							//extra[2*dp_hdr->chunks + 7]=(dp_hdr->len >> 24) & 0x0ff;
							if(0x80==(vpkg_header&0xc0))
							{
                int stream_read_pack_err = 0;
								// last fragment!
								if(dp_hdr->len!=vpkg_length-vpkg_offset)
									mp_msg(MSGT_DEMUX,MSGL_V,"warning! assembled.len=%d  frag.len=%d  total.len=%d  \n",dp->len,vpkg_offset,vpkg_length-vpkg_offset);
									
                if (dp_hdr->len + vpkg_offset > vpkg_length)
                {
#ifdef JZ47_OPT
                  kprintf ("++++ len = %d, dp_hdr->len = %d, vpkg_offset = %d, vpkg_length = %d ++++\n", len, dp_hdr->len, vpkg_offset, vpkg_length);
                	if (len < vpkg_offset - 2 || len > vpkg_offset + 2)
                  {
                    unsigned int pos;
                    pos = stream_tell (demuxer->stream);
                    pos = pos + vpkg_length - dp_hdr->len;
                    stream_seek(demuxer->stream, pos);                    	
                  }
                  else
#endif
								  stream_skip(demuxer->stream, vpkg_offset);
								    
								  dp_hdr->len+=vpkg_offset;
                  len =0;
                  stream_read_pack_err = 1;
                }
                else
                {
								...
								if(priv->video_after_seek){
									...
									dp->pts =
										real_fix_timestamp(priv,dp_data,dp_hdr->timestamp,sh_video->frametime,sh_video->format);
                                                                if (!stream_read_pack_err) ds_add_packet(ds,dp);
								ds->asf_packet=NULL;
//			   F("dp->pts = %f dp->flags = %d\n",dp->pts,dp->flags); 
								// continue parsing
...
//------------------------------------------------------------------------------
					...
					// create new packet!
					int vpkg_length_align = ((vpkg_length + 3) & (~3));
					//dp = new_demux_packet(sizeof(dp_hdr_t)+vpkg_length+8*(1+2*(vpkg_header&0x3F)));
					dp = new_demux_packet(sizeof(dp_hdr_t)+vpkg_length_align+8*(1+2*((vpkg_header)&0x3F)));
					// the timestamp seems to be in milliseconds
					dp->pos = demuxer->filepos;
					if(priv->is_multirate)
						dp->flags = (flags & 0x1) ? 0x10 : 0;
					else
						dp->flags = (flags & 0x2) ? 0x10 : 0;
		
					ds->asf_seq = vpkg_seqnum;
					dp_hdr=(dp_hdr_t*)dp->buffer;
					dp_hdr->flags = dp->flags;
					dp_hdr->chunks=0;
					dp_hdr->timestamp=timestamp;
					dp_hdr->chunktab=sizeof(dp_hdr_t)+vpkg_length_align;
					dp_data=dp->buffer+sizeof(dp_hdr_t);
					//extra=(uint32_t*)(dp->buffer+dp_hdr->chunktab);
					extra=(unsigned char*)(dp->buffer+dp_hdr->chunktab);
...
header_end:
	...
	printf("priv->is_multirate = %d\n",priv->is_multirate);
	printf("priv->duration = %d\n",priv->duration);
	printf("priv->index_table_size[0] = %d\n",priv->index_table_size[0]);
	printf("priv->index_table_size[1] = %d\n",priv->index_table_size[1]);
#if 1
	if(priv->duration == 0)
	{
		if(priv->index_table_size[1] > 0 && 
		   priv->index_table_size[0] > 0)
		{	
			int vstream_id = demuxer->video->id;
			int end_timestamp = priv->index_table[vstream_id][priv->index_table_size[vstream_id] - 1].timestamp;
			int start_timestamp = priv->index_table[vstream_id][0].timestamp;
		
			printf("start_timestamp = %d\n",start_timestamp);
			printf("end_timestamp = %d\n",end_timestamp);
		
			demuxer->seekable = 1;
			priv->duration = (end_timestamp - start_timestamp)/1000;
	...
	{
		// priv->index_table_size[1] || priv->index_table_size[0] 为0，表示不能SEEK
		// 不需要建立索引表
		if(priv->index_table_size[1] == 0 || 
		   priv->index_table_size[0] == 0 || (priv->duration == 0))
		{
			F(">>>>> real duration: %d\n", priv->duration);
			if(!IsOpenForInfo() || (priv->duration == 0))
				generate_index(demuxer);
		}
		F("priv->duration = %d\n",priv->duration);
		//printf("priv->index_table_size[0] = %d\n",priv->index_table_size[0]);
		...
		F("priv->duration = %d\n",priv->duration);
		if(priv->duration > 8)  demuxer->stream->end_pos / priv->duration / 8;
		printf("file_bps = %d\n",file_bps);
		sh_audio_t *sh_audio=demuxer->audio->sh;
		int audio_byte = sh_audio->channels * sh_audio->samplesize * sh_audio->samplerate;
#ifndef __LINUX__
                audio_buf_size = audio_byte / 50;
                if(audio_buf_size < 2048) audio_buf_size = 2048;
#endif
		#if 0
		if(file_bps > 1000 * 900)
		{
		#endif
			audio_buf_count = 8;
			audio_buf_size = (audio_byte / 4); //~0.02s
			
			
		#if 0
		}else if(file_bps > 1000 * 100)
		{
			audio_buf_count = 8;
		...
		ao_data.outburst = (audio_buf_size / audio_buf_count + 31) & (~31);
		ao_data.buffersize = ao_data.outburst * audio_buf_count;
	}			
		
	if(priv->is_multirate == 0)
		frame_dropping = 0;
    return demuxer;
}
...
static void demux_seek_real(demuxer_t *demuxer, float rel_seek_secs, float audio_delay, int flags)
{
    ...
    int streams = 0;
    int retried = 0;
	mp_msg(MSGT_DEMUX,MSGL_INFO,"%5.3f | %5.3f\n",rel_seek_secs,audio_delay);
	F("%5.3f | %5.3f\n",rel_seek_secs,audio_delay);
...
//    printf("streams: %d\n", streams);
	...
	if (streams & 1) {// use the video index if we have one
		...
		{ 	
	    	mp_msg(MSGT_DEMUX,MSGL_INFO,"%5.3f | %5.3f\n",cur_timestamp,priv->index_table[vid][priv->current_vpacket].timestamp);
	    	while ((cur_timestamp - priv->index_table[vid][priv->current_vpacket].timestamp) < - rel_seek_secs * 1000.0f){
	    		priv->current_vpacket -= 1;
		...
		if(priv->is_multirate)
    	{
			unsigned int cur_timestamp =  priv->index_table[demuxer->video->id][priv->current_vpacket].timestamp;
        	priv->a_idx_ptr = 0;
        	while (priv->a_idx_ptr + 1 < priv->index_table_size[demuxer->audio->id] &&
				   cur_timestamp > priv->index_table[demuxer->audio->id][priv->a_idx_ptr + 1].timestamp) 
				priv->a_idx_ptr++;
			F("priv->a_idx_ptr = %d priv->current_vpacket = %d\n",priv->a_idx_ptr,priv->current_vpacket);
			F("priv->index_table[demuxer->audio->id][priv->a_idx_ptr + 1].timestamp = %d\n",priv->index_table[demuxer->audio->id][priv->a_idx_ptr].timestamp);
			F("cur_timestamp = %d\n",cur_timestamp);
			priv->audio_curpos = priv->index_table[demuxer->audio->id][priv->a_idx_ptr].offset;
			priv->audio_need_keyframe = 0;
			F("&&&&&&&& priv->audio_curpos = %d\n",priv->audio_curpos);
		}  
	...
	// }
    F("seek: pos: %d, current packets: a: %d, v: %d\n",
...
static int demux_real_control(demuxer_t *demuxer, int cmd, void *arg)
{
    ...
    switch (cmd) {
    	case DEMUXER_CTRL_IS_SINGLE_SUPPORT:
    	 	if(priv->is_multirate)
	...
	{
		real_index_table_t* idx = FindNextKeyFrameIdx(demuxer);
		*(float *)arg = ((float)(idx->timestamp - cur_timestamp) * 0.001);				
	}
	return DEMUXER_CTRL_OK;
