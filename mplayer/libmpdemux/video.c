...
// Determine image properties:
	...
	case VIDEO_MPEG4: {
		...
		if(!videobuffer) {
			videobuffer=(char*)memalign(32,VIDEOBUFFER_SIZE + FF_INPUT_BUFFER_PADDING_SIZE);
	...
	if(demuxer->file_format==DEMUXER_TYPE_MPEG_ES || 
	   ...
	   || (demuxer->file_format==DEMUXER_TYPE_RTP && demux_is_mpeg_rtp_stream(demuxer))
        ...
        while(videobuf_len<VIDEOBUFFER_SIZE-MAX_VIDEO_PACKET_SIZE){
			...
			// process headers:
			switch(i){
			case 0x1B3: mp_header_process_sequence_header (&picture, &videobuffer[start1]);break;
			case 0x1B5: mp_header_process_extension (&picture, &videobuffer[start1]);break;
			case 0x1B2: process_userdata (&videobuffer[start1], videobuf_len-start1);break;
			case 0x100: picture_coding_type=(videobuffer[start1+1] >> 3) & 7;break;
			}
        }
		...
		*start=videobuffer;
		...
		videobuf_len=0;
	} else if(demuxer->file_format == DEMUXER_TYPE_REAL || demuxer->file_format == DEMUXER_TYPE_AVI)
	{
//		kprintf("d_video->packs = %d\n",d_video->packs);
		in_size=ds_get_packet(d_video,start);
		if(d_video)  picture_coding_type = d_video->flags?1:0;
...
//------------------------ frame decoded. --------------------
    ...
    sh_video->pic_flag = picture_coding_type;
	...
	case DEMUXER_TYPE_LAVF_PREFERRED:
	{
	    ...
	    extern int demuxer_correct_pts;
		
		//F("demuxer_correct_pts = %d\n",demuxer_correct_pts);
		
		if((int)sh_video->fps==1000 || (int)sh_video->fps<=1 || demuxer_correct_pts){
			double next_pts = ds_get_next_pts(d_video);
			double d= (next_pts != MP_NOPTS_VALUE) ? next_pts - d_video->pts : d_video->pts-pts1;
			if(d>=0){
				frame_time = d;
