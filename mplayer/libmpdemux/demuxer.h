...
#undef fast_memcpy
#define fast_memcpy(x,y,z) uc_memcpy(x,y,z)
#endif
...
#define MAX_PACKS 4096
  ...
  #else
     #ifdef JZ4750_OPT
       #define MAX_PACK_BYTES  0x800000 // < 8M
     #else
       #define MAX_PACK_BYTES  0x400000 // < 4M 
     #endif
  #endif
  #undef MAX_PACK_BYTES
...
//#ifdef JZ4740_MTV
#define DEMUXER_TYPE_DAB 45
#define DEMUXER_TYPE_CMMB 46
//#endif
...
#define DEMUXER_CTRL_IS_KEYFRAME       17
#define DEMUXER_CTRL_SET_SKIP          19
#define DEMUXER_CTRL_SKIPFRAME_TIME    22
#define DEMUXER_CTRL_GET_KEYFRAME_TIME 24

#define DEMUXER_CTRL_SKIP_TOKEYFRAME 25


#define DEMUXER_CTRL_IS_SINGLE_SUPPORT 26
...
typedef struct demux_packet_st {
  ...
  int OverDPCache;    //new demux packet is over the demux_packet_buffer range.
  int ReadPacket;     //packet is read in buffer
} demux_packet_t;
...
#if 0 //replace malloc() in *_demux_packet()
demux_packet_t *demux_packet_buffer; //The variable is added to improve playback speed for .vob file.
                                     //It can replace malloc() function in new_demux_packet().
                                     // added by jyu 2008-01-08
...
#else
  if (len > 0 && (dp->buffer = (unsigned char *)memalign(32,len + 8)))
    ;//memset(dp->buffer + len, 0, 8);
#endif
  else
    dp->len = 0;
...
static inline void resize_demux_packet(demux_packet_t* dp, int len)
{
  if(len > 0)
  {
     dp->buffer=(unsigned char *)realloc((void *)dp->buffer,len+8);
  }
  else
  {
...
#endif
//extern unsigned int demuxer_buf[0x100000 / 4];
#ifndef SIZE_MAX
