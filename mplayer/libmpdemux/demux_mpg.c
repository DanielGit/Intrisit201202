...
//      } else if((aid >= 0x80 && aid <= 0x8F) || (aid >= 0x98 && aid <= 0xAF) || (aid >= 0xC0 && aid <= 0xCF)) {
      } else if((aid >= 0x80 && aid <= 0x87) || (aid >= 0xa0 && aid <= 0xAF) || (aid >= 0xC0 && aid <= 0xCF)) {
...
//        printf("\r[%02X][%04X]",c,type);
	  ...
	  if(!((sh_audio_t*)(ds->sh))->codecdata_len){
	      ...
	      ((sh_audio_t*)(ds->sh))->codecdatatype = 1;
...
do{
  ...
  if((head&0xFFFFFF00)!=0x100){
   ...
   while(1)
   {
    ...
    if(head!=0x100){
      head|=c;
      if(mp_check_mp3_header(head))
		  ++num_mp3audio_packets;
      ++skipped; //++demux->filepos;
	  skip_num++;
 	  if( skip_num > 30000 )
 	  {
 		  kprintf(">>>>>>>> SKIP MPGS CHECK skip_num = %d <<<<<<<<\n",skip_num);
 		  return 0;
 	  }
      continue;
    }
  ...
  F("skipped = %d, num_mp3audio_packets = %d",skipped,num_mp3audio_packets);
  if( num_mp3audio_packets >= 256 )
  {
	  demux->stream->eof = 1;
	  kprintf(">>>>>>>> SKIP MPGS CHECK mp3 header num = %d <<<<<<<<\n",num_mp3audio_packets);
	  return 0;
  }
...
//    if(head==0x1BD || (head>=0x1C0 && head<=0x1EF)) demux->synced=3; // PES?
  ...
  else 
  {
	...
	F("num_mp3audio_packets = %d, num_elementary_packets100 = %d",num_mp3audio_packets,num_elementary_packets100);
    if(((num_elementary_packets100>50 && num_elementary_packets101>50) ||
        (num_elementary_packetsPES>50)) && skipped>4000000){
        F("sync_mpeg_ps: seems to be ES/PES stream...\n");
        demux->stream->eof=1;
        break;
    }
	...
	{
        F("sync_mpeg_ps: seems to be MP3 stream...");
        demux->stream->eof=1;
        break;
    }
	F("7");
  }
  F("ret = %d, demux->stream->eof = %d",ret,demux->stream->eof);
} while(ret!=1);
F("9");
