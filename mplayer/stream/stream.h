...
#ifdef JZ4740_MTV
#define STREAMTYPE_DAB 20
#define STREAMTYPE_CMMB 21 
#endif

#ifdef __LINUX__
#define STREAM_BUFFER_SIZE 0x800 /*2048*/
#else
#define STREAM_BUFFER_SIZE 0x800
#endif

#define AUDIO_STREAM_BUFFER_SIZE 0x8000
...
#else
  char *pBuffer;//[STREAM_BUFFER_SIZE>VCD_SECTOR_SIZE?STREAM_BUFFER_SIZE:VCD_SECTOR_SIZE];
#endif
#ifdef JZ4740_MTV
...
#define stream_buf(s,pos) ((unsigned char *)((unsigned int)s->pBuffer + s->pos))
...
#else
  return (s->buf_pos<s->buf_len)?*stream_buf(s,buf_pos++):
    (cache_stream_fill_buffer(s)?*stream_buf(s,buf_pos++):-256);
#endif
...
#else
    memcpy(mem,stream_buf(s,buf_pos),x);
#endif
...
#else
    end = (unsigned char*) memchr((void*)stream_buf(s,buf_pos),'\n',len);
    if(end) len = end - (stream_buf(s,buf_pos)) + 1;
#endif
    if(len > 0 && max > 1) {
...
#else
      memcpy(ptr,stream_buf(s,buf_pos),l);
#endif
      max -= l;
      ptr += l;
    }
