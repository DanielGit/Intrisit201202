...
#endif
#ifndef __LINUX__
extern stream_info_t stream_info_singlefile;
extern stream_info_t stream_info_audiofile;
#endif
stream_info_t* auto_open_streams[] = {
  ...
  &stream_info_file,
#ifndef __LINUX__
  &stream_info_singlefile,
  &stream_info_audiofile,
#endif
  NULL
};
...
#ifndef __LINUX__
//F("options = %s sinfo->info = %s\n",*options,sinfo->info);
  
#if 0
  if(options != 0)
  {
  	if(strcmp(*options,sinfo->info) != 0)
  	{
...
int stream_fill_buffer(stream_t *s){
  ...
  {		
	  switch(s->type){
	  case STREAMTYPE_STREAM:
	    len= s->fill_buffer ? s->fill_buffer(s,&(s->pBuffer),STREAM_BUFFER_SIZE) : 0;
	  case STREAMTYPE_DS:
	    len = demux_read_data((demux_stream_t*)s->priv,&(s->pBuffer),STREAM_BUFFER_SIZE);
	    break;
	...
	}else
	{
		switch(s->type){
	  case STREAMTYPE_STREAM:
	    len= s->fill_buffer ? s->fill_buffer(s,&(s->pBuffer),s->sector_size) : 0;
	    break;
	  case STREAMTYPE_DS:
	    len = demux_read_data((demux_stream_t*)s->priv,&(s->pBuffer),s->sector_size);
	    break;
