...
#ifdef JZ4740_MTV
extern stream_info_t stream_info_dab;
extern stream_info_t stream_info_cmmb;
#endif
...
#else
    len= s->fill_buffer ? s->fill_buffer(s,&(s->pBuffer),STREAM_BUFFER_SIZE) : read(s->fd,&(s->pBuffer),STREAM_BUFFER_SIZE);
    break;
#endif
  case STREAMTYPE_DS:
...
stream_t* new_memory_stream(unsigned char* data,int len){
  ...
  s->pos=len;
  s->pBuffer = data;
  //memcpy(s->pBuffer,data,len);
  printf("no enable new memory stream!\n");
  return s;
}
