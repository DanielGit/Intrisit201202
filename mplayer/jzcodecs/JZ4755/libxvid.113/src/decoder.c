 ...
 *
 * $Id: decoder.c,v 1.36 2009/07/14 01:55:00 zpxu Exp $
 *
 ****************************************************************************/
#ifdef _UCOS_
#include <mplaylib.h>
...
#include "jzsoc/xvid_arch.h"
uint8_t* tcsmloadPTR;
FIFO_MB * aux_pMB;
FIFO_DECODER * aux_dec;
FIFO_MB *dMBbase;
FIFO_MB * fifo_pMB;
FIFO_MB * fofo_pMB;
int32_t * fifo_wp;
uint32_t * fifo_rp;
volatile uint32_t * fifo_start;
volatile uint32_t * task_done;

#endif //JZC_AUX_OPT

#ifdef JZC_TCSM0_OPT
#include "jzsoc/mpeg4_mdma_hw.h"
#define XCHG3(a,b,c,t)     {(t)=(a);(a)=(b);(b)=(c);(c)=(t);}
#define ZERO_ADDR (uint32_t)0x13080000;//real add from 0x130a0000 is 0 of 64k
uint32_t XCHGtmp;
int16_t *block ;
FIFO_MB * p_dMB;
FIFO_MB *dMB0 = (FIFO_MB *)TCSM0_MBARGS_BUF0;
FIFO_MB *dMB1 = (FIFO_MB *)TCSM0_MBARGS_BUF1;
FIFO_MB *dMB2 = (FIFO_MB *)TCSM0_MBARGS_BUF2; 
M4_MDMA_DesNode *MDMA0_ARG;
...
extern FILE *j1_pmon_fp;
extern char* pmon_filename;
J1_PMON_CREAT(vlc);
J1_PMON_CREAT(intra);
...
#ifdef JZC_RTL_SIM
static uint32_t
load_tcsm_bin(uint32_t *load_buf) {
  return 1;
}
#else
#ifdef _UCOS_
...
load_tcsm_bin(uint32_t *load_buf) {
  kprintf("%s:%s\n", __DATE__, __TIME__);
  Load_ProgramBin ("xvid_aux_task.bin", NULL,NULL);
  return 1;
}
#else
#ifdef JZC_FPGA_TEST
...
load_tcsm_bin(uint32_t *load_buf) {
  ...
  int32_t i, len;uint32_t *tcsm;

  fp_text = fopen("./xvid_aux_task.bin", "r+b");
  if (!fp_text) return 0;
  len = fread(load_buf, 4, TCSM1_TASK_LEN, fp_text);
  printf("mpeg4 len of aux task = %d\n", len);
  tcsm = (uint32_t *)TCSM1_VBASE;
  for(i=0; i<len; i++)
    tcsm[i] = load_buf[i];
  // tcsm += len;
  fclose(fp_text);
  jz_dcache_wb(); /*flush cache into TCSM1*/
  printf("%x\n",*tcsm);
  return 1;
}
#else
extern unsigned int xvid_auxcodes_len, xvid_aux_task_codes[];
static uint32_t
load_tcsm_bin(uint32_t *load_buf) {
  int32_t i;
  uint32_t *tcsm;
  ...
  printf("mpeg4 len of aux task = %d\n", xvid_auxcodes_len);
  tcsm = (uint32_t *)TCSM1_VBASE;
  for(i=0; i< xvid_auxcodes_len/4; i++)
    tcsm[i] = xvid_aux_task_codes[i];
  jz_dcache_wb(); /*flush cache into TCSM1*/
  printf("tcsm addr = 0x%08x, *tcsm = 0x%08x\n",tcsm, *tcsm);
  return 1;
}
#endif  // JZC_FPGA_TEST
#endif //_UCOS_
...
decoder_create(xvid_dec_create_t * create)
{
...
#ifdef JZC_TCSM_OPT
  tcsmloadPTR = (uint8_t *)XVIDMALLOC(TCSM1_TASK_LEN * 4 + 4, 0);
  if(!tcsmloadPTR){
    JZPRINT("fifo memory malloc error!\n");
    return XVID_ERR_MEMORY;
  }
  uint8_t *tcsmloadPTR_align = (uint8_t *)(~3 & ((uint32_t)tcsmloadPTR + 3));
  load_tcsm_bin((uint32_t *)tcsmloadPTR_align);
#endif
...
#ifdef JZC_MC_OPT
  {
    //MC init
    int i; 
    XVID_MC_DesNode *mc = (XVID_MC_DesNode *)TCSM1_MC_DES_BUF0_VADDR;
    for(i = 0; i < 12; i ++)
      {
    ...
    mc = (XVID_MC_DesNode *)TCSM1_MC_DES_BUF1_VADDR;
    for(i = 0; i < 12; i ++)
      {
	mc->NodeFlag   = DESP_NODE_HEAD_FG;
	mc->NodeHead   = DESP_NODE_HEAD_LK;
...
#ifdef JZC_TCSM0_OPT
  /* init ARG */
  MDMA0_ARG = (M4_MDMA_DesNode *)((uint32_t)dMB0+1024);
  MDMA0_ARG->STRD = MDMA_STRD(64, 64);
  MDMA0_ARG++;
  MDMA0_ARG->TSA = ZERO_ADDR; 
  MDMA0_ARG->STRD = MDMA_STRD(64, 64);
  MDMA0_ARG->UNIT = MDMA_UNIT(1,64,768);
  ...
  MDMA0_ARG = (M4_MDMA_DesNode *)((uint32_t)dMB2+1024);
  MDMA0_ARG->STRD = MDMA_STRD(64, 64);
  MDMA0_ARG++;
  MDMA0_ARG->TSA = ZERO_ADDR; 
  MDMA0_ARG->STRD = MDMA_STRD(64, 64);
...
#ifdef JZC_AUX_OPT
    /*------------------------------------------------------------------------
     *	    Memory allocation(SDRAM) :  FIFO_MB &FIFO_DECODER
     
     *----------------------------------------------------------------------*/
    //FIFO_MB * const aux_pMB;
    //FIFO_DECODER aux_dec;
# define MAX_MB_NUM 3610
    dMBbase = XVIDMALLOC(sizeof(FIFO_MB)*MAX_MB_NUM+JZ_CACHELINE, 0);//1280*720 pel
   if(!dMBbase){
      JZPRINT("fifo memory malloc error!\n");
      return XVID_ERR_MEMORY;
   }
   aux_pMB = (FIFO_MB *)ALIGN32(dMBbase);
   aux_pMB ++; /*the first aux_pMB space is dummy*/
   
   aux_dec = (FIFO_DECODER *)TCSM1_DFRM_ADDR_VADDR;
   fifo_wp = (uint32_t *)TCSM1_FIFO_WP_VADDR;

   fifo_start =(uint32_t *)TCSM1_FIFO_START_VADDR;
   *fifo_start = tran_addr_for_aux(aux_pMB);
   task_done = TCSM1_VADDR_UNCACHE(TCSM1_TASK_DONE);
   *task_done = 0;
#endif
...
#ifdef JZC_MXU_OPT
     if (dec->quant_type == 0) {
      dequant_h263_intra(&block[i * 64], iQuant, iDcScaler, dec->mpeg_quant_matrices);
    } else {
      dequant_mpeg_intra(&block[i * 64], iQuant, iDcScaler, dec->mpeg_quant_matrices);
    }
  ...
  for (y = 0; y < mb_height; y++) {
	...
	/*get mv*/
        if (mcsel) { 
          ...
          mb->mvs[0].x = mb->mvs[1].x = mb->mvs[2].x = mb->mvs[3].x = 0;
          mb->mvs[0].y = mb->mvs[1].y = mb->mvs[2].y = mb->mvs[3].y = 0;
	  decoder_smbintra(dec, mb, x, y, acpred_flag, cbp, bs, quant,
                  intra_dc_threshold, bound);
	  continue;
        }
          ...
          int coding_type, int quant)
{
...
#ifdef JZC_MAIN_PMON
  {
	...
	fprintf(j1_pmon_fp, "%s\t(size: %d x %d; mb_num: %d) \n",
      ...
      fprintf(j1_pmon_fp,"\t SYNC:\t%lld\n",sync_pmon_val/mb_num);
      //fprintf(j1_pmon_fp,"\t EXPAND:\t%lld\n",expand_pmon_val/mb_num);
      */
      printf("\t EXPAND: %d\n",(int)expand_pmon_val/mb_num);
      printf("\t VLC:\t%d\n",(int)vlc_pmon_val/mb_num);
      printf("\t SYNC:\t%d\n",(int)sync_pmon_val/mb_num);
...
repeat:
  ...
  coding_type = BitstreamReadHeaders(&bs, dec, &rounding,
				     &quant, &fcode_forward, 
				     &fcode_backward, &intra_dc_threshold, &gmc_warp);

#ifdef JZC_TCSM_OPT
  /*write mpeg_matrix to TCSM1 */
  uint16_t *quant_matrix = (uint16_t *)TCSM1_VADDR_UNCACHE(TCSM1_QUANT_MATRIX);
  int temp = 0;
  for(temp = 0;temp < 64;temp++){
    quant_matrix[temp] = dec->mpeg_quant_matrices[temp];
  }
#endif //JZC_TCSM_OPT
  ...
  if (dec->packed_mode && coding_type == N_VOP) {//bframes packed bitstream? (1 = yes)
	...
	decoder_iframe_dcore(dec, &bs, quant, intra_dc_threshold);
      }
#else
      decoder_iframe(dec, &bs, quant, intra_dc_threshold);
#endif //JZC_AUX_OPT
	...
	decoder_pframe_dcore(dec, &bs, rounding, quant,
			     fcode_forward, intra_dc_threshold, NULL);
      }
#else
      decoder_pframe(dec, &bs, rounding, quant,
      ...
      break;
    case S_VOP :
      decoder_sframe(dec, &bs, rounding, quant,
    ...
    if (!(dec->low_delay_default && dec->packed_mode)) {//default is 0 means support B_vop
      if(dec->low_delay) { //l means no B_VOP in VOL
#ifdef IPU_THROUGHT_MODE
	BUFTRAN(dec->qtmp, dec->tmp);
	...
	decoder_bframe_dcore(dec, &bs, quant, fcode_forward, fcode_backward);
      }
#else
      decoder_bframe(dec, &bs, quant, fcode_forward, fcode_backward);
#endif //JZC_AUX_OPT
...
/*
* frome here run to aux,some macro_define must be defined by aux
*
*/
