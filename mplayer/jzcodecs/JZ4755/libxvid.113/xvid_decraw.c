 ...
 *
 * $Id: xvid_decraw.c,v 1.8 2009/06/05 05:12:35 yzhai Exp $
 *
 ****************************************************************************/
...
#ifdef JZC_CRC_VER
	crc_fp = fopen("jz4750e_xvid_crc.log", "aw");
	if(crc_fp == NULL)
	  printf("Error: Open crc log file Failed!\n");
#endif
#ifdef JZC_MAIN_PMON
	pmon_filename = ARG_INPUTFILE;
	j1_pmon_fp = fopen(J1_PMON_FILE_NAME, "aw");
	if(j1_pmon_fp == NULL)
	  printf("Error: Open PMON log file Failed!\n");
#endif // JZC_MAIN_PMON
