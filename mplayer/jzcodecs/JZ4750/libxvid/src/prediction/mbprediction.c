 ...
 *
 * $Id: mbprediction.c,v 1.5 2009/11/04 06:54:05 dyang Exp $
 *
 ****************************************************************************/
...
void __inline
predict_acdc_part1(MACROBLOCK * pMBs,
			 uint32_t x,
			 uint32_t y,
			 uint32_t mb_width,
		   // uint32_t block,
		   ...
		   int32_t *top_quant)
{
	...
	uint32_t index =  mbpos;// x + y * mb_width;	/* current macroblock */
	...
	if (x && mbpos >= bound + 1  &&
		...
		direct[0] = (int16_t*)pMBs[index - 1].pred_values[0];
	...
	if (mbpos >= bound + (int)mb_width &&
		...
		direct[1] = (int16_t*)pMBs[index - mb_width].pred_values[0];
	...
	if (x && mbpos >= bound + (int)mb_width + 1 &&
		...
		direct[2] = (int16_t*)pMBs[index - 1 - mb_width].pred_values[0];
	}

	direct[3] = (int16_t*)pMBs[index].pred_values[0];
