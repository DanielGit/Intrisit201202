...
#if defined(JZ47_OPT) && defined(__LINUX__)
  { MP_CMD_SET_VIDEO, "video", 4, { { MP_CMD_ARG_INT, {0}}, { MP_CMD_ARG_INT, {0}}, { MP_CMD_ARG_INT, {0}}, { MP_CMD_ARG_INT, {0}}, {-1,{0}}  }},
  { MP_CMD_SET_OUTPUT_MODE, "output_mode", 5, { { MP_CMD_ARG_INT, {0}}, { MP_CMD_ARG_INT, {0}}, { MP_CMD_ARG_INT, {0}}, 
...
#ifdef JZ47_OPT
  /* set resize for video out */
  if (cmd-> id == MP_CMD_SET_VIDEO)
  {
    int i, x, y, w, h;
    x = y = w = h = -1;

#if 0
    printf ("==== cmd->name = %s, nargs = %d =====\n", cmd->name, cmd->nargs);
    for (i = 0; i < cmd->nargs; i++)
      printf ("%d, %d\n", i, cmd->args[i].v.i);
#endif

    if (cmd->nargs >= 1) x = cmd->args[0].v.i;
    if (cmd->nargs >= 2) y = cmd->args[1].v.i;
    if (cmd->nargs >= 3) w = cmd->args[2].v.i;
    if (cmd->nargs >= 4) h = cmd->args[3].v.i;
    ipu_outsize_changed(x, y, w, h);
    mp_cmd_free(cmd);
    return NULL;
   }

  /* select output mode for video out */
  if (cmd-> id == MP_CMD_SET_OUTPUT_MODE)
  {
    int i, x, y, w, h, mode;
    x = y = w = h = -1;
    ...
    if (cmd->nargs >= 3) w = cmd->args[2].v.i;
    if (cmd->nargs >= 4) h = cmd->args[3].v.i;
    if (cmd->nargs >= 5) mode = cmd->args[4].v.i;
    ipu_outmode_changed(x, y, w, h, mode);
    mp_cmd_free(cmd);
    return NULL;
   }
#endif
