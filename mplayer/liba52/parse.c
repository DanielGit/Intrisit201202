...
a52_state_t * a52_init (void)
{
    ...
    state->samples = (sample_t *) memalign (32, 256 * 12 * sizeof (sample_t));
	       ...
	       level_t * level, sample_t bias)
{
    ...
    //here is LEVEL *level to fixed point. The following should be done in libmpcodec
    *level =LEVEL(*level);
    ...
    *flags = state->output;
    ...
    state->dynrng = state->level = MUL_L (*level, LEVEL(2));
    //state->dynrng = state->level = MUL_C (LEVEL(*level), 2);
    state->bias = bias;
    state->dynrnge = 1;
...
static inline int16_t dither_gen (a52_state_t * state)
{
    ...
    //return (3 * nstate) >> 2; fix this bug.
    return  nstate;
}
#ifndef LIBA52_FIXED
...
#define COEFF(c,_t,_l,s,e) do {					\
    int32_t t = (int32_t)(_t);		         			\
    level_t l = (_l);						\
    S32MUL(xr1, xr2, t, l);                                     \
    S32EXTR(xr1, xr2,21,31);                             \
    l = S32M2I(xr1);                                          \
    l = l << 1;\
    (c) = l >> e;                                          \
} while (0)
...
#ifdef JZ4750_OPT
static void coeff_get_skip(a52_state_t * state, sample_t * coeff,
		       expbap_t * expbap, quantizer_set_t * quant,
		       level_t level, int dither, int end)
{ 
    int i,q1_ptr,q2_ptr,q4_ptr;
    int8_t * bap;
    bap = expbap->bap;
    q1_ptr = quant->q1_ptr;
    q2_ptr = quant->q2_ptr;
			...
			if (dithflag[ch]){
			    ...
			    samples[ch][i] = (scale_factor[exp[i]] *
					      cplco[ch] * dither_gen (state));
                            samples[ch][i] *= LEVEL_3DB;
#else
			    COEFF (samples[ch][i], dither_gen (state),
				   cplco[ch], scale_factor, exp[i]);
                            samples[ch][i] =MUL(samples[ch][i], LEVEL_3DB_30);
#endif
...
int a52_block (a52_state_t * state)
{
	...
	if (i < 3)
	  ...
	  coeff_get_skip(state, samples + 256 * i, state->fbw_expbap +i, &quant,
		   coeff[i], dithflag[i], state->endmant[i]);
#else
	if (i == 1)
	  coeff_get (state, samples + 256 * i, state->fbw_expbap +i, &quant,
