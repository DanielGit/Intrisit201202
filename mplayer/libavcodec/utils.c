...
void *av_fast_realloc(void *ptr, unsigned int *size, unsigned int min_size)
{
    ...
    *size= FFMAX(33*min_size/32 + 32, min_size);

    return av_realloc(ptr, *size);
}
inline void *av_fast_realloc_static(void *ptr, unsigned int *size, unsigned int min_size)
{
    if(min_size < *size)
        return ptr;
    ...
    *size= FFMAX(33*min_size/32 + 32, min_size);
    ...
    return av_realloc(ptr, *size);
#else
    return uc_realloc_static(ptr, *size);
#endif
...
#else
	  void *ptr = uc_memalign_static(32,size);
#endif
    if(ptr){
#ifdef __LINUX__ 
        array_static =av_fast_realloc(array_static, &allocated_static, sizeof(void*)*(last_static+1));
#else
        array_static =av_fast_realloc_static(array_static, &allocated_static, sizeof(void*)*(last_static+1));
#endif
        if(!array_static)
            return NULL;
...
#else
            array_static[i] = uc_realloc_static(array_static[i], size);
#endif
...
#ifdef USE_IPU_THROUGH_MODE
    if ((unsigned int)buf->data[0] != disp_buf0 && (unsigned int)buf->data[0] != disp_buf1)
    {
...
#else
    if (avctx->codec->avclose)
        avctx->codec->avclose(avctx);
#endif
