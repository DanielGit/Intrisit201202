...
#else
static int32_t cos_mod_fix[544];
#endif
...
typedef struct {
    ...
    int32_t scalefactor_adj_fix[DCA_PRIM_CHANNELS_MAX][DCA_ABITS_MAX];   ///< scale factor adjustment
    ...
    int16_t  lfe_data_fixed[2 * DCA_SUBSUBFAMES_MAX * DCA_LFE_MAX *
                   2 /*history */ ];    ///< Low frequency effect data
    ...
    int32_t subband_samples_hist_fix[DCA_PRIM_CHANNELS_MAX][DCA_SUBBANDS][4];
    int32_t subband_fir_hist_fix[DCA_PRIM_CHANNELS_MAX][512];
    int32_t subband_fir_noidea_fix[DCA_PRIM_CHANNELS_MAX][64];
...
static int dca_parse_frame_header(DCAContext * s)
{
...
#else
    static const int32_t adj_table_fix[4] = {  67108864,  75497472,  83886080,  96468992};
#endif
    ...
    for (j = 0; j < 11; j++)
        for (i = 0; i < s->prim_channels; i++){
            ...
            s->scalefactor_adj_fix[i][j] = 1<<26;
        }
...
#else
                s->scalefactor_adj_fix[i][j] =adj_table_fix[get_bits(&s->gb, 2)]; 
#endif
            }
...
#else
static void qmf_32_subbands_fix(DCAContext * s, int chans,
                            int32_t samples_in_fix[32][8], int32_t *samples_out,
                            float scale, float bias)
{
    int i, j, k, j2;
    int chindex = 0, subindex;
    ...
    int32_t *prCoeff_fix;
    int32_t raXin_fix[32];
    int32_t *subband_fir_hist2_fix = s->subband_fir_noidea_fix[chans];
    int hist_index= s->hist_index[chans];

    /* Select filter */
    if (!s->multirate_inter){    /* Non-perfect reconstruction */
        prCoeff_fix = (int32_t *) fir_32bands_nonperfect_fix;
    }else{                       /* Perfect reconstruction */
        prCoeff_fix = (int32_t *) fir_32bands_perfect_fix;
    }

    /* Reconstructed channel sample index */
    for (subindex = 0; subindex < 8; subindex++) {
        int32_t t1_fix, t2_fix, sum_fix[16], diff_fix[16];
        int32_t *subband_fir_hist_fix = s->subband_fir_hist_fix[chans]+hist_index;
        int32_t *hist2_0=subband_fir_hist2_fix-1;
        int32_t *hist2_16=subband_fir_hist2_fix+15;
        for (i = 0; i < s->subband_activity[chans]; i++){
            if((i-1)&2) raXin_fix[i] = -samples_in_fix[i][subindex];
            else        raXin_fix[i] =  samples_in_fix[i][subindex];
        }
        for (; i < 32; i++)
            raXin_fix[i] = 0;

        ff_imdct_half_fix(&s->imdct, subband_fir_hist_fix, raXin_fix);
        for (i = 0; i < 16; i++){
            int32_t *coe_0=prCoeff_fix+i;
            int32_t *hist_15=subband_fir_hist_fix+15-i;
            int32_t *hist_0=subband_fir_hist_fix+i;
            S32LDI(xr10,hist2_0,0x4);
            S32LDI(xr11,hist2_16,0x4);
            S32MUL(xr1,xr2,coe_0[0],hist_15[0]);
            S32MUL(xr3,xr4,coe_0[16],hist_0[0]);
            S32MUL(xr5,xr6,coe_0[32], hist_0[16]);
            S32MUL(xr7,xr8,coe_0[48], hist_15[16]);
            D32ASUM_AA(xr1,xr10,xr11,xr3);
            for (j = 64; j < 512-hist_index; j += 64){
                S32MSUB(xr1,xr2,coe_0[j],hist_15[j]);
                S32MADD(xr3,xr4,coe_0[16+j],hist_0[j]);
                S32MADD(xr5,xr6,coe_0[32+j], hist_0[16+j]);
                S32MADD(xr7,xr8,coe_0[48+j], hist_15[16+j]);
            }
            for (     ; j < 512; j += 64){
                S32MSUB(xr1,xr2,*(coe_0+j) ,*(hist_15+j-512));
                S32MADD(xr3,xr4,*(coe_0+16+j),*(hist_0+j-512));
                S32MADD(xr5,xr6,*(coe_0+32+j),*(hist_0+16+j-512));
                S32MADD(xr7,xr8,*(coe_0+48+j),*(hist_15+16+j-512));
            }
            S32STDV(xr1,samples_out,i,2);
            S32STDV(xr3,samples_out,i+16,2);
            S32STD(xr5,hist2_0,0);
            S32STD(xr7,hist2_16,0);
        }
        samples_out += 32;
...
#else
#define MIX_REAR1(samples, si1, rs, coef) \
     samples[i]     += MULFIX_31(samples[si1], coef[rs][0]); \
     samples[i+256] += MULFIX_31(samples[si1], coef[rs][1]);

#define MIX_REAR2(samples, si1, si2, rs, coef) \
     samples[i]     += MULFIX_31(samples[si1], coef[rs][0])+MULFIX_31(samples[si2], coef[rs+1][0]); \
     samples[i+256] += MULFIX_31(samples[si1], coef[rs][1])+MULFIX_31(samples[si2], coef[rs+1][1]);

#define MIX_FRONT3(samples, coef) \
    t = samples[i]; \
    samples[i]     = MULFIX_31(t, coef[0][0]) + MULFIX_31(samples[i+256], coef[1][0]) + MULFIX_31(samples[i+512], coef[2][0]); \
    samples[i+256] = MULFIX_31(t, coef[0][1]) + MULFIX_31(samples[i+256], coef[1][1])+ MULFIX_31(samples[i+512], coef[2][1]);
#define DOWNMIX_TO_STEREO(op1, op2) \
    for(i = 0; i < 256; i++){ \
        op1 \
        op2 \
...
static void dca_downmix_fix(int32_t *samples_fix, int srcfmt,
                        int downmix_coef[DCA_PRIM_CHANNELS_MAX][2])
{
    int i;
    int32_t t;
    int32_t coef_fix[DCA_PRIM_CHANNELS_MAX][2];

    for(i=0; i<DCA_PRIM_CHANNELS_MAX; i++) {
        coef_fix[i][0] = dca_downmix_coeffs_fix[downmix_coef[i][0]];
        coef_fix[i][1] = dca_downmix_coeffs_fix[downmix_coef[i][1]];
    }
    ...
    case DCA_STEREO:
        break;
    case DCA_3F:
        DOWNMIX_TO_STEREO(MIX_FRONT3(samples_fix, coef_fix),);
        break;
    case DCA_2F1R:
        DOWNMIX_TO_STEREO(MIX_REAR1(samples_fix, i + 512, 2, coef_fix),);
        break;
    case DCA_3F1R:
        DOWNMIX_TO_STEREO(MIX_FRONT3(samples_fix, coef_fix),
                          MIX_REAR1(samples_fix, i + 768, 3, coef_fix));
        break;
    case DCA_2F2R:
        DOWNMIX_TO_STEREO(MIX_REAR2(samples_fix, i + 512, i + 768, 2, coef_fix),);
        break;
    case DCA_3F2R:
        DOWNMIX_TO_STEREO(MIX_FRONT3(samples_fix, coef_fix),
                          MIX_REAR2(samples_fix, i + 768, i + 1024, 3, coef_fix));
        break;
    }
}
#endif
...
#else
static int dca_subsubframe_fix(DCAContext * s)
{
    int k, l, m;
    int subsubframe = s->current_subsubframe;

    /* FIXME */
    int32_t *quant_step_table_fix;
    int32_t subband_samples_fix[DCA_PRIM_CHANNELS_MAX][DCA_SUBBANDS][8];
    ...
    /* Select quantization step size table */
    if (s->bit_rate == 0x1f){
        quant_step_table_fix = (int32_t *)lossless_quant_d_fix;
    }else{
        quant_step_table_fix = (int32_t *)lossy_quant_d_fix;
    }
    for (k = 0; k < s->prim_channels; k++) {
        for (l = 0; l < s->vq_start_subband[k]; l++) {
            ...
            int32_t quant_step_size_fix= quant_step_table_fix[abits];
            int32_t rscale_fix;
            int32_t *sample_kl=subband_samples_fix[k][l];
            /*
             * Determine quantization index code book and its type
             */
             ...
             */
            if(!abits){
                memset(sample_kl, 0, 8 * sizeof(sample_kl[0]));
            }else if(abits >= 11 || !dca_smpl_bitalloc[abits].vlc[sel].table){
                if(abits <= 7){
                    /* Block code */
                    int block_code1, block_code2, size, levels;
                    ...
                    /* no coding */
                    for (m = 0; m < 8; m++){
                        sample_kl[m] = get_sbits(&s->gb, abits - 3);
                    }
                }
            }else{
                /* Huffman coded */
                for (m = 0; m < 8; m++){
                    sample_kl[m] = get_bitalloc(&s->gb, &dca_smpl_bitalloc[abits], sel);
                }
            }
            ...
            if (s->transition_mode[k][l] &&
                subsubframe >= s->transition_mode[k][l]){
                rscale_fix=MULFIX_26(quant_step_size_fix, s->scale_factor[k][l][1]);
            }else{
                rscale_fix=MULFIX_26(quant_step_size_fix, s->scale_factor[k][l][0]);
            }

            rscale_fix = MULFIX_26(rscale_fix,s->scalefactor_adj_fix[k][sel]);

            for (m = 0; m < 8; m++){
                sample_kl[m] *= rscale_fix;
            }
            ...
            if (s->prediction_mode[k][l]) {
                ...
                for (m = 0; m < 8; m++) {
                    for (n = 1; n <= 4; n++)
                        if (m >= n){
                            sample_kl[m] +=
                                (((int64_t)(sample_kl[m - n])*
                                 adpcm_vb[s->prediction_vq[k][l]][n - 1])>>13);
                        }else if (s->predictor_history){
                            sample_kl[m] +=
                                (((int64_t)(s->subband_samples_hist_fix[k][l][m - n +4])*
                                 adpcm_vb[s->prediction_vq[k][l]][n - 1] )>>13);
                        }
        ...
        for (l = s->vq_start_subband[k]; l < s->subband_activity[k]; l++) {
            ...
            for (m = 0; m < 8; m++) {
                subband_samples_fix[k][l][m] =
                    (((int64_t)(s->scale_factor[k][l][0])
                    *high_freq_vq[s->high_freq_vq[k][l]][subsubframe * 8 + m]  )>>4);
            }
        }
    }
    ...
    for (k = 0; k < s->prim_channels; k++)
        for (l = 0; l < s->vq_start_subband[k]; l++){
            memcpy(s->subband_samples_hist_fix[k][l], &subband_samples_fix[k][l][4],
                        4 * sizeof(subband_samples_fix[k][l][0]));
        }
    /* 32 subbands QMF */
    for (k = 0; k < s->prim_channels; k++) {
/*        static float pcm_to_double[8] =
            {32768.0, 32768.0, 524288.0, 524288.0, 0, 8388608.0, 8388608.0};*/
         qmf_32_subbands_fix(s, k, subband_samples_fix[k], &s->samples_fix[256 * k],
                            2.0 / 3 /*pcm_to_double[s->source_pcm_res] */ ,
                            0 /*s->bias */ );
    }
    ...
    if (s->prim_channels > dca_channels[s->output & DCA_CHANNEL_MASK]) {
        dca_downmix_fix(s->samples_fix, s->amode, s->downmix_coef);
    }
                            ...
                            uint8_t * buf, int buf_size)
{
    ...
    *data_size = 0;
    for (i = 0; i < (s->sample_blocks / 8); i++) {
        ...
        for (j = 0; j < 256; j++) {
            for (k = 0; k < channels; k++)
                samples[k] = (int16_t)(s->samples_fix[j + k * 256]);
            samples += channels;
        }
...
static void pre_calc_cosmod(DCAContext * s)
{
    ...
    for (j = 0, k = 0; k < 16; k++)
        for (i = 0; i < 16; i++)
            cos_mod_fix[j++] = LEVEL_31(cos((2 * i + 1) * (2 * k + 1) * M_PI / 64));
    for (k = 0; k < 16; k++)
        for (i = 0; i < 16; i++)
            cos_mod_fix[j++] = LEVEL_31(cos((i) * (2 * k + 1) * M_PI / 32));

    for (k = 0; k < 16; k++)
        cos_mod_fix[j++] = LEVEL_26((8 * cos((2 * k + 1) * M_PI / 128)));

    for (k = 0; k < 16; k++)
        cos_mod_fix[j++] = LEVEL_26(-(8 * sin((2 * k + 1) * M_PI / 128)));
#endif
    cosmod_inited = 1;
}
