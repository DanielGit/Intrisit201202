...
#ifndef JZ4750_OPT
//This function is moved to vp6.c and vp5.c through function pointer.
//This funcion pointer is declared in vp56.h by jyu 2009.4.3
...
#ifdef JZ4750_OPT
    if (dx)  s->edge_filter_x(s, yuv +         10-dx ,      1, stride, t);
    if (dy)  s->edge_filter_y(s, yuv + stride*(10-dy), stride,      1, t);
#else
...
#ifdef JZ4750_OPT
        put_pixels16_c(s->edge_emu_buffer,
                                    src + s->block_offset[b] + (dy-2)*stride + (dx-2),
                                    stride, 12);
#else
...
extern unsigned int disp_buf0, disp_buf1;
    AVFrame dispfrm[2];
    AVFrame *dispfrmp[2];
#endif
...
#ifdef USE_IPU_THROUGH_MODE
        if(s->framep[VP56_FRAME_GOLDEN]->data[0]==disp_buf1){
          FFSWAP(AVFrame *, s->framep[VP56_FRAME_GOLDEN],
                              dispfrmp[1]);
        }
        if (s->framep[VP56_FRAME_GOLDEN]->data[0]){
        if (s->framep[VP56_FRAME_GOLDEN]->data[0]!=s->framep[VP56_FRAME_CURRENT]->data[0]){
...
#ifdef USE_IPU_THROUGH_MODE
        if(s->framep[VP56_FRAME_PREVIOUS]->data[0]==disp_buf1){
          FFSWAP(AVFrame *, s->framep[VP56_FRAME_PREVIOUS],
                              dispfrmp[0]);
        }
...
/*
int vp56_get_buffer(AVCodecContext *s, AVFrame *pic){
    int i;
    int w= s->width;
    int h= s->height;
    InternalBuffer *buf;
...
#ifdef JZ4740_IPU
#ifdef __LINUX__
            buf->base[i]= jz4740_alloc_frame (16, size[i]+16);
#else
...
*/
void vp56_release_buffer(AVCodecContext *s, AVFrame *pic){
    int i;
...
#ifdef USE_IPU_THROUGH_MODE
    avctx->release_buffer= vp56_release_buffer;
#endif
...
#ifdef USE_IPU_THROUGH_MODE
    dispfrmp[0] = &dispfrm[0];
    dispfrmp[1] = &dispfrm[1];
#endif 
...
#ifdef JZ4750_OPT
static void put_pixels16_c(uint8_t *dst, const uint8_t *src, int stride, int h){
...
static void put_no_rnd_pixels8_l2_c(uint8_t *dst, const uint8_t *src1, const uint8_t *src2,
         int stride, int h){
    int i;
    for(i=0; i<h; i++){
        uint32_t a,b;
        a= (((const struct unaligned_32 *) (&src1[i*stride ]))->l);
        b= (((const struct unaligned_32 *) (&src2[i*stride ]))->l);
        *((uint32_t*)&dst[i*stride ]) = no_rnd_avg32(a, b);
        a= (((const struct unaligned_32 *) (&src1[i*stride+4]))->l);
        b= (((const struct unaligned_32 *) (&src2[i*stride+4]))->l);
