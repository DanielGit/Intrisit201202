...
typedef struct FFTComplexFix {
    int refix, imfix;
} FFTComplexFix;

typedef struct FFTContextFix {
    int nbits;
    int inverse;
    uint16_t *revtab;
    FFTComplexFix *exptab;
    FFTComplexFix *exptab1; /* only used by SSE code */
} FFTContextFix;

/* MDCT computation */

typedef struct MDCTContextFix {
...
//FIXME the following 3 tables should be shared between decoders
    ...
    int noise_table_fix[NOISE_TAB_SIZE];
    int noise_index;
    /* lsp_to_curve tables */
    int noise_mult_fix; /* XXX: suppress that and integrate it in the noise array */
    /* lsp_to_curve tables */
    int lspfix_cos_table[BLOCK_MAX_SIZE];
