...
//    MDCTContext         mdct_ctx;
    ...
    CookMDCTContext     mdct_ctx;

    DECLARE_ALIGNED_16(CookFFTSample, mdct_tmp[1024]);  /* temporary storage for imlt */
    CookLevel *         mlt_window;
    /* gain buffers */
    cook_gains          gains1;
    cook_gains          gains2;
    int                 gain_1[9];
    ...
    DECLARE_ALIGNED_16(CookFFTSample,mono_mdct_output[2048]);
    CookFFTSample               mono_previous_buffer1[1024];
    CookFFTSample               mono_previous_buffer2[1024];
    CookFFTSample               decode_buffer_1[1024];
    CookFFTSample               decode_buffer_2[1024];
    CookFFTSample              decode_buffer_0[1060]; /* static allocation for joint decode */

    CookFFTSample *cplscales[5];
} COOKContext;
...
static void init_gain_table(COOKContext *q) {
    ...
    for (i=0 ; i<23 ; i++) {
        q->gain_table[i] = FIXEDSAMPLE(
                             pow((double)q->pow2tab[i+52] ,
...
static int init_cook_mlt(COOKContext *q) {
    ...
    if ((q->mlt_window = av_malloc(sizeof(CookLevel)*mlt_size)) == 0)
      return -1;
    ...
    for(j=0 ; j<mlt_size ; j++){
        q->mlt_window[j] =
                 COOKFIXED31(sin((j + 0.5) * alpha) * sqrt(2.0 / q->samples_per_channel));
   }
    /* Initialize the MDCT. */
    if( Cook_mdct_init(&q->mdct_ctx, av_log2(mlt_size)+1,1)) {
...
static CookFFTSample *maybe_reformat_buffer32 (COOKContext *q, CookFFTSample *ptr, int n)
{
    if (1)
        return ptr;
}
                         ...
                         CookFFTSample * mlt_buffer2)
{
    ...
    int de_fix;
    CookFFTSample f1,f2;
    CookFFTSample * cplscale;

    memset(decouple_tab, 0, sizeof(decouple_tab));
    memset(decode_buffer, 0, sizeof(decode_buffer));

    /* Make sure the buffers are zeroed out. */
    memset(mlt_buffer1,0, 1024*sizeof(CookFFTSample ));
    memset(mlt_buffer2,0, 1024*sizeof(CookFFTSample ));
...
//   q->decouple (q, i, f1, f2, decode_buffer, mlt_buffer1, mlt_buffer2);
        for (j=0 ; j<SUBBAND_SIZE ; j++) {
           tmp_idx = ((q->js_subband_start +i)*SUBBAND_SIZE)+j;
           de_fix = decode_buffer[tmp_idx];
           mlt_buffer1[SUBBAND_SIZE*i+ j] = MUL(f1, de_fix);
           mlt_buffer2[SUBBAND_SIZE*i+ j] = MUL(f2, de_fix);
        }
        idx = (1 << q->js_vlc_bits) - 1;
    }
}
...
{
    int j;
    CookFFTSample *output = q->mono_mdct_output + q->samples_per_channel;
    /* Clip and convert floats to 16 bits.
     */
    for (j = 0; j < q->samples_per_channel; j++) {
        out[chan + q->nb_channels * j] =
#if 1
          av_clip_int16((output[j]>>8));
#else
...
static inline void
mlt_compensate_output(COOKContext *q, CookFFTSample *decode_buffer,
                      cook_gains *gains, CookFFTSample *previous_buffer,
