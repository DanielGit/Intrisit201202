...
#ifdef JZ4750_OPT
static void vp5_edge_filter_x(vp56_context_t *s, uint8_t *yuv,
...
static void vp5_edge_filter_y(vp56_context_t *s, uint8_t *yuv,
                             int pix_inc, int line_inc, int t)
{
    int pix2_inc = 2 * pix_inc;
    int i, v;
...
static int vp5_decode_init(AVCodecContext *avctx)
{
    ...
    s->adjust = vp5_adjust;
#ifdef JZ4750_OPT
    s->edge_filter_x = vp5_edge_filter_x;
    s->edge_filter_y = vp5_edge_filter_y;
#endif
