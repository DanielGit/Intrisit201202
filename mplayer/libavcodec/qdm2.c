...
#define SB_DITHERING_NOISE_FIX(sb,noise_idx) QDM2_MUL_31(noise_table_fix[(noise_idx)++],sb_noise_attenuation_fix[(sb)])
...
#ifdef JZ47_OPT
    int32_t level_fix;
    int32_t *samples_im_fix;
    int32_t *samples_re_fix;
    int32_t *table_fix;
#else
    float level;
    float *samples_im;
...
typedef struct {
#ifdef JZ47_OPT
    QDM2Complex_fix complex_fix[256 + 1];
    int32_t       samples_im_fix[MPA_MAX_CHANNELS][256];
    int32_t       samples_re_fix[MPA_MAX_CHANNELS][256];
#else
...
typedef struct {
    ...
    int fft_level_exp[6];
#ifdef JZ47_OPT
    FFTComplex_fix exptab_fix[128];
    FFTContext_fix fft_ctx_fix;
    FFTComplex_fix exptab_fix_512[512];
    FFTComplex_fix exptab_fix_8[8];
#else
    FFTContext fft_ctx;
    ...
    int compressed_size;
#ifdef JZ47_OPT
    int32_t output_buffer_fix[1024];
#else
...
#ifdef JZ47_OPT
    int32_t tone_level_fix[MPA_MAX_CHANNELS][30][64];
#else
...
#ifdef JZ47_OPT
static int32_t noise_table_fix[4096];
static int32_t f2i_scale_fix = (1 << (FRAC_BITS - 15));
static int32_t type34_delta_fix[10];
static int32_t type30_dequant_fix[8];
static int32_t dequant_1bit_fix[2][3];
static int32_t type10_f_fix;
static int32_t noise_samples_fix[128];
#else
...
static void rnd_table_init(void) {
    ...
    for(i = 0; i < 4096 ;i++) {
        ...
        noise_table_fix[i] = (int32_t)((delta * (float)(((int32_t)random_seed >> 16) & 0x00007FFF)- 1.0) * 1.3 * f2i_scale + 0.5);
    }
#else //JZ47_OPT
    for(i = 0; i < 4096 ;i++) {
        random_seed = random_seed * 214013 + 2531011;
...
static void init_noise_samples(void) {
    ...
    for (i = 0; i < 128;i++) {
        random_seed = random_seed * 214013 + 2531011;
        noise_samples_fix[i] = (int)((delta * (float)((random_seed >> 16) & 0x00007fff) - 1.0)*f2i_scale + 0.5);
    }
#else
    for (i = 0; i < 128;i++) {
        random_seed = random_seed * 214013 + 2531011;
...
static void build_sb_samples_from_noise (QDM2Context *q, int sb)
{
    ...
    for (ch = 0; ch < q->nb_channels; ch++)
        for (j = 0; j < 64; j++) {
            q->sb_samples[ch][j * 2][sb] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx) * q->tone_level_fix[ch][sb][j];
            q->sb_samples[ch][j * 2 + 1][sb] = q->sb_samples[ch][j*2][sb];
        }
#else 
    for (ch = 0; ch < q->nb_channels; ch++)
        for (j = 0; j < 64; j++) {
            ...
            for (ch = 0; ch < q->nb_channels; ch++)
                for (i = 0; i < 64; i++) {
                    ...
                    if (q->tone_level_idx[ch][sb][i] < 0)
                        q->tone_level_fix[ch][sb][i] = 0;
                    else
                        q->tone_level_fix[ch][sb][i] = fft_tone_level_table_fix[0][q->tone_level_idx[ch][sb][i] & 0x3f];
#else
                    if (q->tone_level_idx[ch][sb][i] < 0)
                ...
                for (ch = 0; ch < q->nb_channels; ch++)
                    for (i = 0; i < 64; i++) {
                        ...
                        if ((tmp < 0) || (!q->superblocktype_2_3 && !tmp))
                            ...
                            q->tone_level_fix[ch][sb][i] = fft_tone_level_table_fix[tab][tmp & 0x3f];
#else
                        if ((tmp < 0) || (!q->superblocktype_2_3 && !tmp))
                            q->tone_level[ch][sb][i] = 0;
                        else
 ...
 */
#ifdef JZ47_OPT
static void synthfilt_build_sb_samples_fix (QDM2Context *q, GetBitContext *gb, int length, int sb_min, int sb_max)
{
    int sb, j, k, n, ch, run, channels;
    int joined_stereo, zero_encoding, chs;
    int type34_first;
    int32_t type34_div_fix = 0;
    int32_t type34_predictor_fix;
    int32_t samples_fix[10], sign_bits_fix[16];
    ...
    for (sb = sb_min; sb < sb_max; sb++) {
        ...
        for (ch = 0; ch < channels; ch++) {
            ...
            for (j = 0; j < 128; ) {
                switch (q->coding_method[ch][sb][j / 2]) {
                        ...
                        if (BITS_LEFT(length,gb) >= 10) {
                            if (zero_encoding) {
                                for (k = 0; k < 5; k++) {
                                    if ((j + 2 * k) >= 128)
                                        break;
                                    samples_fix[2 * k] = get_bits1(gb) ? dequant_1bit_fix[joined_stereo][2 * get_bits1(gb)] : 0;
                                }
                            } else {
                                n = get_bits(gb, 8);
                                for (k = 0; k < 5; k++)
                                    samples_fix[2 * k] = dequant_1bit_fix[joined_stereo][random_dequant_index[n][k]];
                            }
                            for (k = 0; k < 5; k++)
                                samples_fix[2 * k + 1] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                        } else {
                            for (k = 0; k < 10; k++)
                                samples_fix[k] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                        }
                        run = 10;
                        break;
                    ...
                    case 10:
                        if (BITS_LEFT(length,gb) >= 1) {
                            int32_t f_fix = type10_f_fix;

                            if (get_bits1(gb))
                                f_fix = -f_fix;
                            f_fix -= noise_samples_fix[((sb + 1) * (j +5 * ch + 1)) & 127] * 9 / 40;
                            samples_fix[0] = f_fix;
                        } else {
                            samples_fix[0] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                        }
                        run = 1;
                        break;
                        ...
                        if (BITS_LEFT(length,gb) >= 10) {
                            if (zero_encoding) {
                                for (k = 0; k < 5; k++) {
                                    if ((j + k) >= 128)
                                        break;
                                    samples_fix[k] = (get_bits1(gb) == 0) ? 0 : dequant_1bit_fix[joined_stereo][2 * get_bits1(gb)];
                                }
                            } else {
                                n = get_bits (gb, 8);
                                for (k = 0; k < 5; k++)
                                    samples_fix[k] = dequant_1bit_fix[joined_stereo][random_dequant_index[n][k]];
                            }
                        } else {
                            for (k = 0; k < 5; k++)
                                samples_fix[k] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                    ...
                    case 24:
                        if (BITS_LEFT(length,gb) >= 7) {
                            n = get_bits(gb, 7);
                            for (k = 0; k < 3; k++)
                                samples_fix[k] = (random_dequant_type24[n][k] - 2) * f2i_scale_fix / 2;
                        } else {
                            for (k = 0; k < 3; k++)
                                samples_fix[k] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                        }
                    ...
                    case 30:
                        if (BITS_LEFT(length,gb) >= 4)
                            samples_fix[0] = type30_dequant_fix[qdm2_get_vlc(gb, &vlc_tab_type30, 0, 1)];
                        else
                            samples_fix[0] = SB_DITHERING_NOISE_FIX(sb,q->noise_idx);
                        run = 1;
                        break;
                    ...
                    case 34:
                        if (BITS_LEFT(length,gb) >= 7) {
                            if (type34_first) {
                                type34_div_fix = (1 << get_bits(gb, 2));
                                samples_fix[0] = QDM2_FIXED_18(get_bits(gb, 5) - 16) / 15;
                                type34_predictor_fix = samples_fix[0];
                                samples_fix[0] >>= 10;
                                type34_first = 0;
                            } else {
                                samples_fix[0] = type34_delta_fix[qdm2_get_vlc(gb, &vlc_tab_type34, 0, 1)] / type34_div_fix + type34_predictor_fix;
                                type34_predictor_fix = samples_fix[0];
                                samples_fix[0] >>= 10;
                            }
                        } else {
                ...
                if (joined_stereo) {
                   int32_t tmp_fix[10][MPA_MAX_CHANNELS];
                   for (k = 0; k < run; k++) {
                        tmp_fix[k][0] = samples_fix[k];
                        tmp_fix[k][1] = (sign_bits_fix[(j + k) / 8]) ? -samples_fix[k] : samples_fix[k];
                    }
                    for (chs = 0; chs < q->nb_channels; chs++)
                        for (k = 0; k < run; k++)
                            if ((j + k) < 128)
                                q->sb_samples[chs][j + k][sb] = q->tone_level_fix[chs][sb][((j + k)/2)] * tmp_fix[k][chs];
                } else {
                    for (k = 0; k < run; k++)
                        if ((j + k) < 128)
                            q->sb_samples[ch][j + k][sb] = q->tone_level_fix[ch][sb][(j + k)/2] * samples_fix[k];
                }
...
#ifdef JZ47_OPT
    synthfilt_build_sb_samples_fix(q, &gb, length, 0, 8);
#else
...
static void process_subpacket_12 (QDM2Context *q, QDM2SubPNode *node, int length)
{
    ...
    init_get_bits(&gb, ((node == NULL) ? empty_buffer : node->packet->data), ((node == NULL) ? 0 : node->packet->size*8));
#ifdef JZ47_OPT
    synthfilt_build_sb_samples_fix(q, &gb, length, 8, QDM2_SB_USED(q->sub_sampling));
#else
...
#ifdef JZ47_OPT
static void qdm2_fft_generate_tone_fix (QDM2Context *q, FFTTone *tone)
{
   int i;
   QDM2Complex_fix cfix;
   int level_fix, ffix[6];

    tone->phase += tone->phase_shift;

    /* calculate current level (maximum amplitude) of tone */
    level_fix = QDM2_MUL_26(fft_tone_envelope_table_fix[tone->duration][tone->time_index] , tone->level_fix);
    cfix.im = QDM2_MUL_31(level_fix, q->exptab_fix_512[(tone->phase+512)%512].im);
    cfix.re = QDM2_MUL_31(level_fix, q->exptab_fix_512[(tone->phase+512)%512].re);
    /* generate FFT coefficients for tone */
    if (tone->duration >= 3 || tone->cutoff >= 3) {
        tone->samples_im_fix[0] += cfix.im;
        tone->samples_re_fix[0] += cfix.re;
        tone->samples_im_fix[1] -= cfix.im;
        tone->samples_re_fix[1] -= cfix.re;
    } else {
        ffix[1] = -tone->table_fix[4];
        ffix[0] =  tone->table_fix[3] - tone->table_fix[0];
        ffix[2] =  QDM2_FIXED_26(1.0) - tone->table_fix[2] - tone->table_fix[3];
        ffix[3] =  tone->table_fix[1] + tone->table_fix[4] - QDM2_FIXED_26(1.0);
        ffix[4] =  tone->table_fix[0] - tone->table_fix[1];
        ffix[5] =  tone->table_fix[2];
        for (i = 0; i < 2; i++) {
            tone->samples_re_fix[fft_cutoff_index_table[tone->cutoff][i]] += 
               QDM2_MUL_26(cfix.re, ffix[i]);
            tone->samples_im_fix[fft_cutoff_index_table[tone->cutoff][i]] +=
               QDM2_MUL_26(cfix.im, (tone->cutoff <= i) ? -ffix[i] : ffix[i]);
        }
        for (i = 0; i < 4; i++) {
            tone->samples_re_fix[i] += QDM2_MUL_26(cfix.re , ffix[i+2]);
            tone->samples_im_fix[i] += QDM2_MUL_26(cfix.im , ffix[i+2]);
        }
    }
...
#endif //JZ47_OPT
#ifdef JZ47_OPT
static void qdm2_fft_tone_synthesizer_fix (QDM2Context *q, int sub_packet)
{
    int i, j, ch;

    for (ch = 0; ch < q->channels; ch++) {
        memset(q->fft.samples_im_fix[ch], 0, q->fft_size * sizeof(int32_t));
        memset(q->fft.samples_re_fix[ch], 0, q->fft_size * sizeof(int32_t));
    }
    ...
    if (q->fft_coefs_min_index[4] >= 0)
        for (i = q->fft_coefs_min_index[4]; i < q->fft_coefs_max_index[4]; i++) {
            ...
            level_fix = (q->fft_coefs[i].exp < 0) ? 0.0 : fft_tone_level_table_fix[q->superblocktype_2_3 ? 0 : 1][q->fft_coefs[i].exp & 63];
            cfix.re = QDM2_MUL_31(level_fix, q->exptab_fix_8[q->fft_coefs[i].phase%8].re);
            cfix.im = QDM2_MUL_31(level_fix, q->exptab_fix_8[q->fft_coefs[i].phase%8].im);
            q->fft.samples_re_fix[ch][q->fft_coefs[i].offset + 0] += cfix.re;
            q->fft.samples_im_fix[ch][q->fft_coefs[i].offset + 0] += cfix.im;
            q->fft.samples_re_fix[ch][q->fft_coefs[i].offset + 1] -= cfix.re;
            q->fft.samples_im_fix[ch][q->fft_coefs[i].offset + 1] -= cfix.im;
        }

    /* generate existing FFT tones */
    for (i = q->fft_tone_end; i != q->fft_tone_start; ) {
        qdm2_fft_generate_tone_fix(q, &q->fft_tones[q->fft_tone_start]);
        ...
        if (q->fft_coefs_min_index[i] >= 0) {
            for (j = q->fft_coefs_min_index[i]; j < q->fft_coefs_max_index[i]; j++) {
                ...
                if (offset < q->frequency_range) {
                    ...
                    tone.level_fix = (q->fft_coefs[j].exp < 0) ? 0.0 : fft_tone_level_table_fix[q->superblocktype_2_3 ? 0 : 1][q->fft_coefs[j].exp & 63];
                    tone.samples_im_fix = &q->fft.samples_im_fix[ch][offset];
                    tone.samples_re_fix = &q->fft.samples_re_fix[ch][offset];
                    tone.table_fix = (int32_t*)fft_tone_sample_table_fix[i][q->fft_coefs[j].offset - (offset << four_i)];
                    tone.phase = 64 * q->fft_coefs[j].phase - (offset << 8) - 128;
                    tone.phase_shift = (2 * q->fft_coefs[j].offset + 1) << (7 - four_i);
                    tone.duration = i;
                    tone.time_index = 0;

                    qdm2_fft_generate_tone_fix(q, &tone);
                }
            }
...
#endif //JZ47_OPT
#ifdef JZ47_OPT
static void qdm2_calculate_fft_fix (QDM2Context *q, int channel, int sub_packet)
{
    const int n = 1 << (q->fft_order - 1);
    const int n2 = n >> 1;
    int i, j;
    int32_t cfix, sfix, f0fix, f1fix, f2fix, f3fix;
    const int32_t gain_fix = (q->channels == 1 && q->nb_channels == 2) ? 2 : 1;
    
    for (i=1; i < n2; i++) {
        j  = (n - i);
        cfix = q->exptab_fix[i].re;
        sfix = -q->exptab_fix[i].im;
        f0fix = (q->fft.samples_re_fix[channel][i] - q->fft.samples_re_fix[channel][j]);
        f1fix = (q->fft.samples_im_fix[channel][i] + q->fft.samples_im_fix[channel][j]);
        f2fix = (q->fft.samples_re_fix[channel][i] + q->fft.samples_re_fix[channel][j]);
        f3fix = (q->fft.samples_im_fix[channel][i] - q->fft.samples_im_fix[channel][j]);
        q->fft.complex_fix[i].re =  QDM2_MUL_31(sfix,f0fix) 
                                  - QDM2_MUL_31(cfix,f1fix) + f2fix;
        q->fft.complex_fix[i].im =  QDM2_MUL_31(cfix,f0fix) 
                                  + QDM2_MUL_31(sfix,f1fix) + f3fix;
        q->fft.complex_fix[j].re = -QDM2_MUL_31(sfix,f0fix) 
                                  + QDM2_MUL_31(cfix,f1fix) + f2fix;
        q->fft.complex_fix[j].im =  QDM2_MUL_31(cfix,f0fix) 
                                  + QDM2_MUL_31(sfix,f1fix) - f3fix;
    }
    q->fft.complex_fix[ 0].re =  q->fft.samples_re_fix[channel][ 0] <<1;
    q->fft.complex_fix[ 0].im =  q->fft.samples_re_fix[channel][ 0] <<1;
    q->fft.complex_fix[n2].re =  q->fft.samples_re_fix[channel][n2] <<1;
    q->fft.complex_fix[n2].im = -q->fft.samples_im_fix[channel][n2] <<1;
    fft_permute_fix(&q->fft_ctx_fix, (FFTComplex_fix *) q->fft.complex_fix);
    fft_calc_fix (&q->fft_ctx_fix, (FFTComplex_fix *) q->fft.complex_fix);
    for (i = 0; i < ((q->fft_frame_size + 15) & ~15); i++)
        q->output_buffer_fix[q->channels * i + channel] += (((int32_t *) q->fft.complex_fix)[i])>>gain_fix;
}
#else //JZ47_OPT
...
static void qdm2_synthesis_filter (QDM2Context *q, int index)
{
    ...
    for (ch = 0; ch < q->channels; ch++)
        for (i = 0; i < q->frame_size; i++)
            q->output_buffer_fix[q->channels * i + ch] += (samples[q->nb_channels * sub_sampling * i + ch] >> (sizeof(OUT_INT)*8-16));
#else
    for (ch = 0; ch < q->channels; ch++)
        for (i = 0; i < q->frame_size; i++)
...
#ifdef JZ47_OPT
    fft_init_fix(&s->fft_ctx_fix, s->fft_order - 1, 1);
#else
...
#ifdef JZ47_OPT
        s->exptab_fix[i].re = QDM2_FIXED_31(cos(alpha));
        s->exptab_fix[i].im = QDM2_FIXED_31(sin(alpha));
#else
...
#ifdef JZ47_OPT
    for(i=0;i<512;i++){
        alpha = 2 * M_PI * (float)i / 512.0;
        s->exptab_fix_512[i].re = QDM2_FIXED_31(cos(alpha));
        s->exptab_fix_512[i].im = QDM2_FIXED_31(sin(alpha));
    }
    for(i=0;i<8;i++){
        alpha = 2 * M_PI * (float)i / 8.0;
        s->exptab_fix_8[i].re = QDM2_FIXED_31(cos(alpha));
        s->exptab_fix_8[i].im = QDM2_FIXED_31(sin(alpha));
    }
#endif
#ifdef JZ47_OPT
    for(i=0; i<8; i++){
       type30_dequant_fix[i] = (int)(type30_dequant[i]*f2i_scale + 0.5);
    }
    for(i=0; i<10; i++){
       type34_delta_fix[i] = QDM2_FIXED_18(type34_delta[i]);
    }
    for(i = 0; i< 2; i++){
     for(tmp = 0; tmp < 3; tmp++){
       dequant_1bit_fix[i][tmp]=(int)(dequant_1bit[i][tmp]*f2i_scale+0.5);
     }
    }
    type10_f_fix = (int32_t)(0.81 * f2i_scale + 0.5);
#endif
    qdm2_init(s);
...
//  dump_context(q);
...
#ifdef JZ47_OPT
    memmove(q->output_buffer_fix, &q->output_buffer_fix[frame_size], frame_size * sizeof(int32_t));
    memset(&q->output_buffer_fix[frame_size], 0, frame_size * sizeof(int32_t));
#else
...
#ifdef JZ47_OPT
        qdm2_fft_tone_synthesizer_fix(q, q->sub_packet);
#else
    ...
    for (ch = 0; ch < q->channels; ch++) {
#ifdef JZ47_OPT
        qdm2_calculate_fft_fix(q, ch, q->sub_packet);
#else
