...
static unsigned int init_pm_level = -1;
...
#define Is_Fast_Codes(codesname,mpi,name,size) (codesname && (strcmp(codesname,name) == 0) && (mpi->width * mpi->height >= size))
...
static int current_out_width = 0,current_out_height = 0;
static void jz47_ipu_init (struct vf_instance_s* vf,mp_image_t *mpi)
{
	F("1\n");
	SwsContext *c = vf->priv->ctx;
	...
	c->dstW = outW;
	c->dstH = outH;
	current_out_width = outW;
	current_out_height = outH;
	if(ipu_inited)
	  PEND_IPU();
	switch(ipu_size_cfg)
	{
...
//   	dmpi, (mpi->flags&MP_IMGFLAG_DRAW_CALLBACK),visual);
    //#undef printf
    // printf("current_out_height = %d mpi->height = %d\n",MP_ALIGN(mpi->height),MP_ALIGN(current_out_height));
    
    if((MP_ALIGN(mpi->width) != MP_ALIGN(current_out_width)) || (MP_ALIGN(mpi->height) != MP_ALIGN(current_out_height)))
    {
        display_change = 1;
        visual = 0;
        mpi->pict_type = 0;
