 ...
 * $Id: decode.c,v 1.4 2009/04/16 16:18:22 jyu Exp $
 */

#include "config.h"

#include <mplaylib.h>	/* memcmp/memset, try to remove */
#include <mplaylib.h>
#include <inttypes.h>
...
#define BUFFER_SIZE  1222656 //(1194 * 1024)
...
void mpeg2_reset (mpeg2dec_t * mpeg2dec, int full_reset)
{
    ...
    mpeg2dec->decoder.quant_store= NULL;
...
#define DCT_TCSM_TBL0 (0xF4000000)
#define DCT_TCSM_TBL1 (0xF4000100)
#define DCT_TCSM_TBL2 (0xF4000200)
#define DCT_TCSM_TBL3 (0xF4000300)
#define DCT_TCSM_TBL4 (0xF4000400)
//#define DCT_TCSM_TBL5 (0xF4000500)
//#define DCT_TCSM_TBL5 (0xF4000500)
    DCTtab *DCT_13_tcsm;
    DCTtab *DCT_15_tcsm;
    DCTtab *DCT_B14_10_tcsm;
    DCTtab *DCT_16_tcsm;
    DCTtab *DCT_B14_8_tcsm;
  //  DCTtab *DCT_B15_8_tcsm;
  //  DCTtab *DCT_B14AC_5_tcsm;

void  table_tcsm_init()
{
    int i;
    DCT_13_tcsm = (DCTtab *)DCT_TCSM_TBL0;
    DCT_15_tcsm = (DCTtab *)DCT_TCSM_TBL1;
    DCT_B14_10_tcsm = (DCTtab *)DCT_TCSM_TBL2;
    DCT_16_tcsm = (DCTtab *)DCT_TCSM_TBL3;
    DCT_B14_8_tcsm = (DCTtab *)DCT_TCSM_TBL4;
    //DCT_B14AC_5_tcsm = (DCTtab *)DCT_TCSM_TBL5;
    // DCT_B15_8_tcsm = (DCTtab *)DCT_TCSM_TBL5;

    for( i = 0; i < 48; i ++) {
    DCT_13_tcsm[i].run   = DCT_13[i].run;
    DCT_13_tcsm[i].level = DCT_13[i].level;
    DCT_13_tcsm[i].len   = DCT_13[i].len;
    
    DCT_15_tcsm[i].run   = DCT_15[i].run;
    DCT_15_tcsm[i].level = DCT_15[i].level;
    DCT_15_tcsm[i].len   = DCT_15[i].len;
    }
 
    for( i = 0; i < 8; i ++) {
    DCT_B14_10_tcsm[i].run =  DCT_B14_10[i].run;
    DCT_B14_10_tcsm[i].level =  DCT_B14_10[i].level;
    DCT_B14_10_tcsm[i].len = DCT_B14_10[i].len;
    }

    for( i = 0; i < 32; i ++) {
    DCT_16_tcsm[i].run   = DCT_16[i].run;
    DCT_16_tcsm[i].level = DCT_16[i].level;
    DCT_16_tcsm[i].len   = DCT_16[i].len;
    }

    for( i = 0; i < 36; i ++) {
    DCT_B14_8_tcsm[i].run =  DCT_B14_8[i].run;
    DCT_B14_8_tcsm[i].level =  DCT_B14_8[i].level;
    DCT_B14_8_tcsm[i].len = DCT_B14_8[i].len;
    }
   /*
    for( i = 0; i < 252; i ++) {
    DCT_B15_8_tcsm[i].run =  DCT_B15_8[i].run;
    DCT_B15_8_tcsm[i].level =  DCT_B15_8[i].level;
    DCT_B15_8_tcsm[i].len = DCT_B15_8[i].len;
    }
     
    for( i = 0; i < 27; i ++) {
    DCT_B14AC_5_tcsm[i].run =  DCT_B14AC_5[i].run;
    DCT_B14AC_5_tcsm[i].level =  DCT_B14AC_5[i].level;
    DCT_B14AC_5_tcsm[i].len = DCT_B14AC_5[i].len;
    }
   */
}
