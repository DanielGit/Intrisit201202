 ...
 * $Id: slice.c,v 1.3 2009/01/08 01:17:14 xjyu Exp $
 */
...
const int  patter_table[16] = {1,2,4,8,16,32,(1 << 30),(2 << 30),(3 << 30),(1 << 26),(2 << 26),(4 << 26),(8 << 26),(16 << 26),(32 << 26),
 (63 << 26)};

#ifdef VLC_TBL_IN_TCSM
extern  DCTtab *DCT_13_tcsm;
extern  DCTtab *DCT_15_tcsm;
extern  DCTtab *DCT_B14_10_tcsm;
extern  DCTtab *DCT_16_tcsm;
extern  DCTtab *DCT_B14_8_tcsm;
//extern  DCTtab *DCT_B14AC_5_tcsm;
//extern  DCTtab *DCT_B15_8_tcsm;
#endif
				 ...
				 const uint16_t * const quant_matrix)
{
	...
	normal_code:
	   ...
	   // tab = DCT_13_tcsm + (UBITS (bit_buf, 13) - 16);
	   ...
	   // tab = DCT_15_tcsm + (UBITS (bit_buf, 15) - 16);
				 ...
				 const uint16_t * const quant_matrix)
{
	...
	if (bit_buf >= 0x04000000) {
	      tab = DCT_B15_8 + (UBITS (bit_buf,8 ) - 4);
            //tab = DCT_B15_8_tcsm + (UBITS (bit_buf,8 ) - 4);
            i_noreorder;
            my_run = (uint32_t)tab->run;
            my_len = (uint32_t)tab->len;
            my_lvl = (uint32_t)tab->level;
   ...
   // idct_row_max_intra = val>>3;
    idct_row_max_intra = S32M2I(xr11);
}
#else
...
static int get_non_intra_block (mpeg2_decoder_t * const decoder,
 ...
 {  int i;
    ...
    while (1) {
      int32_t my_run, my_len, my_lvl, tmp;
	if (bit_buf >= 0x28000000) {
             // tab = DCT_B14AC_5_tcsm + (UBITS (bit_buf, 5) - 5);
	    tab = DCT_B14AC_5 + (UBITS (bit_buf, 5) - 5);
	entry_1:
	    i_noreorder;
	    my_run = (uint32_t)tab->run;
    ...
    entry_2:
	if (bit_buf >= 0x04000000) {
          //  tab = DCT_B14_8_tcsm + (UBITS (bit_buf, 8) - 4);
	    tab = DCT_B14_8 + (UBITS (bit_buf, 8) - 4);
	    i_noreorder;
	    my_run = (uint32_t)tab->run;
	    my_len = (uint32_t)tab->len;
	    ...
	    continue;
              ...
              tab = DCT_B14_10_tcsm + (UBITS (bit_buf, 10) - 8);
	    //tab = DCT_B14_10 + (UBITS (bit_buf, 10) - 8);
            i_noreorder;
	    my_run = (uint32_t)tab->run;
	    my_len = (uint32_t)tab->len;
		...
		goto normal_code;
            ...
            tab = DCT_16_tcsm + UBITS (bit_buf, 16);
	    //tab = DCT_16 + UBITS (bit_buf, 16);
            bit_buf <<= 16;
	    GETWORD (bit_buf, bits + 16, bit_ptr);
	    i_noreorder;
				...
				const uint16_t * const quant_matrix)
{
	...
	normal_code:
	    ...
	    bit_buf <<= tab->len;
	    bits += tab->len + 1;
	    val = (((tab->level << 1) + 1) * quant_matrix[j]) >> 5;
            S32MAX(xr11, xr11, xr10);
	    /* if (bitstream_get (1)) val = -val; */
            tmp = SBITS (bit_buf, 1);
	    val = (val ^ tmp) - tmp;
	...
	if (bit_buf >= 0x04000000) {
	    ...
	    DUMPBITS (bit_buf, bits, 12);
	    NEEDBITS (bit_buf, bits, bit_ptr);
	    val = ((SBITS (bit_buf, 12) + SBITS (bit_buf, 1)) << 1) + 1;
	    val = ((val * quant_matrix[j]) >> 5);
		      ...
		      uint8_t * forward_fbuf[3], uint8_t * backward_fbuf[3])
{
    ...
    decoder->slice_stride = (stride << 4);
    decoder->slice_uv_stride =
	decoder->slice_stride >> (2 - decoder->chroma_format);
    decoder->limit_x = ((decoder->width) << 1) - 32;
    decoder->limit_y_16 = (height << 1) - 32;
    decoder->limit_y_8 = (height << 1) - 16;
...
#define bit_ptr (decoder->bitstream_ptr)
    ...
    #ifdef JZ4750_OPT    
    S32LDDR(xr1,buffer,0x0);
    #else
    decoder->bitstream_buf =
      (buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) | buffer[3];
    #endif
     decoder->bitstream_buf = S32M2I(xr1);
	...
	if (macroblock_modes & MACROBLOCK_INTRA) {
		...
		DCT_stride = ((decoder->stride) << 1);
	    } else {
		DCT_offset = ((decoder->stride) << 3);
		...
		if (likely (decoder->chroma_format == 0)) {
		    ...
		    if (coded_block_pattern & patter_table[0])
			slice_non_intra_DCT (decoder, 0, dest_y, DCT_stride);
		    if (coded_block_pattern & patter_table[1])
			slice_non_intra_DCT (decoder, 0, dest_y + 8,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[2])
			slice_non_intra_DCT (decoder, 0, dest_y + DCT_offset,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[3])
			slice_non_intra_DCT (decoder, 0,
					     dest_y + DCT_offset + 8,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[4])
			slice_non_intra_DCT (decoder, 1,
					     decoder->dest[1] + (offset >> 1),
					     decoder->uv_stride);
		    if (coded_block_pattern & patter_table[5])
		    ...
		    int offset;
		    uint8_t * dest_y;
		    coded_block_pattern |= bit_buf & patter_table[8];
		    DUMPBITS (bit_buf, bits, 2);
			...
			slice_non_intra_DCT (decoder, 2,
					     decoder->dest[2] + (offset >> 1),
					     DCT_stride);
		    if (coded_block_pattern & patter_table[7])
			slice_non_intra_DCT (decoder, 1,
					     decoder->dest[1] + DCT_offset,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[6])
			slice_non_intra_DCT (decoder, 2,
		    ...
		    coded_block_pattern |= bit_buf & patter_table[15];
		    ...
		    if (coded_block_pattern & patter_table[14])
			slice_non_intra_DCT (decoder, 1, dest_u + DCT_offset,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[13])
			slice_non_intra_DCT (decoder, 2, dest_v + DCT_offset,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[12])
			slice_non_intra_DCT (decoder, 1, dest_u + 8,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[11])
			slice_non_intra_DCT (decoder, 2, dest_v + 8,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[10])
			slice_non_intra_DCT (decoder, 1,
					     dest_u + DCT_offset + 8,
					     DCT_stride);
		    if (coded_block_pattern & patter_table[9])
			slice_non_intra_DCT (decoder, 2,
					     dest_v + DCT_offset + 8,
					     DCT_stride);
		}
