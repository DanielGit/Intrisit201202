...
#if 0
	int percent;
  percent = demuxer_get_percent_pos(mpctx->demuxer);	
  float f = (float)percent * demuxer_get_time_length(mpctx->demuxer) / 100.0;				  
		//return (int)demuxer_get_current_time(mpctx->demuxer);				  
	return (int)f;
#endif
}
...
static void initmplayer()
{
	//***************************************************//
	 ...
	 if(videoshow)
	 	BUFF_SetBufSize(0xfc00*2);
	 else
	 	BUFF_SetBufSize(0x80000);
	 	createclass();
...
static double written_audio_pts(sh_audio_t *sh_audio, demux_stream_t *d_audio)
{
   ...
   //F("%d\n",(int)(a_pts));
    ...
    a_pts -= (double)sh_audio->a_buffer_len / (double)sh_audio->o_bps;

    // Data that was ready for ao but was buffered because ao didn't fully
    // accept everything to internal buffers yet
    a_pts -= (double)sh_audio->a_out_buffer_len * playback_speed / (double)ao_data.bps;
...
static float timing_sleep(float time_frame)
{
	...
	//dsqiu
	float margin = softsleep ? 0.011 : 0.001;
...
#ifdef USE_16M_SDRAM
load_plugin ("table", "av_codec_mtvtable", 0, FFMPEG_PLUGIN_TABLE);
#endif
...
// and now ? -- Pontscho
    ...
    if(rtc_fd<0)
#endif /* HAVE_RTC */
      mp_msg(MSGT_CPLAYER, MSGL_V, "Using %s timing111\n",
...
F("30\n");
F("%x %x %d %d %d %s\n",mpctx->stream,mpctx->file_format,audio_id,video_id,dvdsub_id,filename);
...
//F("pts: %f :%f\n",mpctx->sh_audio->pts,mpctx->sh_video->pts);   
if(mpctx->sh_audio->pts-mpctx->sh_video->pts>2.0)
		sync_mtv();
...
void *load_plugin (char *drv_name, char *dll_name, char video_plg, unsigned int arg0)
{
  ...
  char *vd_prefix="mpcodecs_vd_", *ad_prefix="mpcodecs_ad_mtv";
