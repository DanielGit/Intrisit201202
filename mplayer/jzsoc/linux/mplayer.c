...
#define AUX_CTRL_ADDR    0xB3800000
#define AUX_RESET()               \
  ({                                      \
    volatile unsigned int * addr;         \
    addr = (unsigned int *)AUX_CTRL_ADDR;   \
    *addr = 1;                                    \
  })
...
static int first_update_playing = 1;
...
extern void jz47_exit_memalloc(void *);
...
static int sleep_until_update(float *time_frame, float *aq_sleep_time)
{
    ...
    *time_frame -= GetRelativeTime(); // reset timer
    if (first_update_playing)
      *time_frame = 0;
    first_update_playing = 0;
...
if((int)seek_to_sec && !total_frame_cnt)
	decoded_frame = NULL;
		...
		{				
					...
					#endif		
			disp_buf0 = disp_buf1;
						disp_buf1 = (unsigned int)((mp_image_t *)decoded_frame)->planes[0];
...
int seekmode = 0;
...
#else
  cpm_base = mmap((void *)0, CPM__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 
		 mmapfd, CPM__OFFSET);
#ifndef JZ4760_OPT
  ...
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(IDCT_CLOCK);
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(DBLK_CLOCK);
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(ME_CLOCK);
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(MC_CLOCK);
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(IPU_CLOCK);
#else
  REG32(cpm_base + CPM_CLKGR_OFFSET) &= ~(JZ4760_IPU_CLOCK);
  REG32(cpm_base + CPM_CLKGR1_OFFSET) &= ~(JZ4760_VIDEO_DECODER_CLOCK);
#endif
  ...
  dblk_base = mmap((void *)0, DBLK__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 
		   mmapfd, DBLK__OFFSET);
  idct_base = mmap((void *)0, IDCT__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 
		   mmapfd, IDCT__OFFSET);	
