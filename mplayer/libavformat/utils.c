...
static AVInputFormat *av_probe_input_format2(AVProbeData *pd, int is_opened, int *score_max)
{
    ...
    for(fmt1 = first_iformat; fmt1 != NULL; fmt1 = fmt1->next) {
	 ...
	 score_ext = 0;
        if (fmt1->read_probe) {
            score_probe = fmt1->read_probe(pd);
         } 
	if (fmt1->extensions) {
	...
	score = (score_probe > score_ext) ? score_probe : score_ext;
        if (score > *score_max) {
            *score_max = score;
            fmt = fmt1;
        }
...
/*            if(!st->time_base.num)
    ...
    for(;;) {
        ...
        // h264 decoder is supported in flv file, so add 2 lines following by jyu 2009.03.06
        if(ret == AVERROR(EAGAIN))
            continue;
        if (ret < 0) {
            /* EOF or error */
