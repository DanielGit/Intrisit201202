...
//            num_val = av_int2dbl();
        ...
        default: //unsupported type, we couldn't skip
        	kprintf("unsupported type, skiped: %d\n", amf_type);
            return -1;
    }
    ...
    if(depth == 1 && key) { //only look for metadata values when we are not nested and key != NULL
        ...
        if(amf_type == AMF_DATA_TYPE_BOOL) {
            ...
            else if(!strcmp(key, "width")  && vcodec && num_val > 0) vcodec->width  = (int)num_val;
            else if(!strcmp(key, "height") && vcodec && num_val > 0) vcodec->height = (int)num_val;
            ...
            else if(!strcmp(key, "audiosamplesize") && acodec && (int)num_val > 0) {
...
static int flv_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 for(;;){
    ...
    //av_log(s, AV_LOG_INFO, "type:%d, size:%d, dts:%d, pos=%d\n", type, size, (int)dts, pos);
    if (url_feof(&s->pb))
        return AVERROR(EIO);
    url_fskip(&s->pb, 3); /* reserved */
    flags = 0;
    ...
    if (type == FLV_TAG_TYPE_AUDIO) {
        ...
        if (type == FLV_TAG_TYPE_META && size > 13+1+4)
            ...
            av_log(s, AV_LOG_INFO, "skipping flv packet: type %d, size %d, flags %d\n", type, size, flags);
    skip:
        url_fseek(&s->pb, next, SEEK_SET);
