...
1.26.00 20030113 TB     Support for trial version added.
                        ...
                        specification.
1.28.00 20030207 TB     jzfs_RmDir support added.
                        jzfs_IoCtl command FS_CMD_GET_DISKFREE added.
1.28.01 20030311 TB     jzfs_Write/jzfs_Read preformance improved.
1.30.00 20030530 TB     Switch FS_FAT_DISKINFO added.
                        Logical Block Read Cache implemented.
                        No open file bug when using jzfs_Open(name,"r+") 
                        corrected.
...
1.31.00 20030807 TB     Internal test version
                        ...
                        are no longer shown by jzfs_ReadDir.
                        jzfs_Write performance improved by optimizing
                        directory access.
                        ...
                        in ROM now.
                        jzfs_Remove reports error when file does not
                        exist or cannot be removed.
                        jzfs_IoCtl commands FS_CMD_READ_SECTOR and
...
1.32.00 20030910 TB/TW  Official successor of 1.30
                        ...
                        jzfs_Write performance improved by avoiding 
...
1.34.01 20031015 TW     jzfs_Write cluster allocation failed.
...
FS_FILE             *jzfs_Open(const char *pFileName, const char *pMode);
void                jzfs_Close(FS_FILE *pFile);
FS_size_t           jzfs_Read(void *pData, FS_size_t Size, FS_size_t N, FS_FILE *pFile);
FS_size_t           jzfs_Write(const void *pData, FS_size_t Size, FS_size_t N, FS_FILE *pFile);
...
int                 jzfs_Seek(FS_FILE *pFile, FS_i32 Offset, int Whence);
FS_i32              jzfs_Tell(FS_FILE *pFile);
...
FS_i16              jzfs_Error(FS_FILE *pFile);
void                jzfs_ClearError(FS_FILE *pFile);
...
int                 jzfs_Remove(const char *pFileName);
...
int                 jzfs_IoCtl(const char *pDevName, FS_i32 Cmd, FS_i32 Aux, void *pBuffer);
...
JZFS_DIR              *jzfs_OpenDir(const char *pDirName);
int                 jzfs_CloseDir(JZFS_DIR *pDir);
struct FS_DIRENT    *jzfs_ReadDir(JZFS_DIR *pDir);
void                jzfs_RewindDir(JZFS_DIR *pDir);
int                 jzfs_Mkdir(const char *pDirName);
int                 jzfs_RmDir(const char *pDirName);
